...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        mondays_range = [date for date in date_range if date.weekday() == 0]

        
        for objdate in mondays_range:    
        ...
        #    for org in orgs: 
                    ...
                    for emp in emp_list:
                                    ...
                                    try:    
                                        ...
                                        if PresentDays >= 4  and PresentDays != 7 and AbsentDays <=2 and NextDayStatus == None :
                                            
                                                Absentdateobj = attendance_data_absent_days = Attendance_Data.objects.filter(EmployeeCode=EmployeeCode, OrganizationID=OrganizationID, IsDelete=False, Status='Absent', Date__in = dates).first()

                                                Absentdate = Absentdateobj.Date
                                                
                                                try:
                                                    WeekOffDetails.objects.get(Emp_Code=EmployeeCode, WeekoffDate=Absentdate,OrganizationID=OrganizationID,IsDelete=False)
                                                except  WeekOffDetails.DoesNotExist :    
                                                    WeekOffDetails.objects.create(Emp_Code=EmployeeCode, WeekoffDate=Absentdate,OrganizationID= OrganizationID)
                                                    att_update =  Attendance_Data.objects.get(EmployeeCode = EmployeeCode , Date = Absentdate,OrganizationID=OrganizationID,IsDelete=False)
