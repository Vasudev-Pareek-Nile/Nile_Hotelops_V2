...
def KraEnrty(request):
    ...
    # print("selected_year type::", type(selected_year))
    # print("selected_month::", selected_month)
    # print("selected_month type::",type(selected_month))
    ...
    with connection.cursor() as cursor:
        # cursor.execute("EXEC KRA_Entry_Select @OrganizationID=%s,  @EntryYear = %s, @EntryMonth=%s, @UserID=%s, @UID=%s, @EmpCode=%s", [ OrganizationID , selected_year, selected_month, UserID,UserID, SessionEmpCode])
        cursor.execute(
            "EXEC KRA_Entry_Select @OrganizationID=%s, @EntryYear=%s, @EntryMonth=%s, @UserID=%s, @UID=%s, @EmpCode=%s",
...
#     # from_year_str = request.GET.get('FromYear')  # e.g., "04-2019"
#     # to_year_str = request.GET.get('ToYear')      # e.g., "07-2025"

#     # print("from_year_str::", from_year_str)
#     # print("to_year_str::", to_year_str)
...
#     # ✅ Keep columns like 'Jan-2025' — DO NOT rename them in pivot_rating
#     # pivot_rating.rename(columns=month_map, inplace=True)  ←❌ REMOVE this line


#     # Generate all Month-Year columns between the range
#     full_month_range = generate_month_range(from_month, from_year, to_month, to_year)

#     # Create empty DataFrame with required columns if not exist
...
def KRA_Target_Report_View(request):
    ...
    return render(request, "EMP_PAY/MoveToPayroll_Template/KRA_Target_Report.html", context)


# def KRA_Target_Report_Api_View(request):
#     current_date = datetime.now()
#     OrganizationID = request.session["OrganizationID"]
#     Year = int(request.GET.get('year', current_date.year))
#     Month = int(request.GET.get('month_no', current_date.month))
