...
def homedeta(request):
    ...
    # Corporate
    corp_divisions = CorporateDivisionMaster.objects.filter(IsDelete=False).order_by('Order')
    corp_departments = CorporateDepartmentMaster.objects.filter(IsDelete=False).order_by('Order')
    corp_designations = CorporateDesignationMaster.objects.filter(IsDelete=False).order_by('Order')
    
    # print("corp_designations:",corp_designations)
    ...
    MergedReportingtoDesignation = chain(reportings, corp_designations)
    ...
    context = {
        ...
        'corp_designations': corp_designations,
...
def resolve_division_name(division_id):
    if not division_id:
        return None

    div = OnRollDivisionMaster.objects.filter(
        id=division_id, IsDelete=False
    ).values_list('DivisionName', flat=True).first()

    if div:
        return div

    return CorporateDivisionMaster.objects.filter(
        id=division_id, IsDelete=False
    ).values_list('DivisionName', flat=True).first()

# resolve_department_name
def resolve_department_name(department_id):
    if not department_id:
        return None

    dept = OnRollDepartmentMaster.objects.filter(
        id=department_id, IsDelete=False
    ).values_list('DepartmentName', flat=True).first()

    if dept:
        return dept

    return CorporateDepartmentMaster.objects.filter(
        id=department_id, IsDelete=False
    ).values_list('DepartmentName', flat=True).first()


# resolve_designation_name
def resolve_designation_name(designation_id):
    if not designation_id:
        return None

    desi = OnRollDesignationMaster.objects.filter(
        id=designation_id, IsDelete=False
    ).values_list('designations', flat=True).first()

    if desi:
        return desi

    return CorporateDesignationMaster.objects.filter(
        id=designation_id, IsDelete=False
    ).values_list('designations', flat=True).first()

# resolve_designation_name
def resolve_designation_name_corp(designation_id, new_division_id):
    if not designation_id:
        return None
    ...
    desi = OnRollDesignationMaster.objects.filter(
        id=designation_id, 
        IsDelete=False,
        OnRollDivisionMaster_id = new_division_id
    ).values_list('designations', flat=True).first()
    ...
    return CorporateDesignationMaster.objects.filter(
        id=designation_id, 
        IsDelete=False,
        CorporateDivisionMaster_id = new_division_id
    ).values_list('designations', flat=True).first()

def Change_Direct_Reporting_To(
    user_department_id,
    user_designation_id,
    new_division_id,
    new_department_id,
    ...
    user_dept_name = resolve_department_name(user_department_id)
    user_desi_name = resolve_designation_name(user_designation_id)

    new_division_name = resolve_division_name(new_division_id)
    new_department_name = resolve_department_name(new_department_id)
    new_designation_name = resolve_designation_name_corp(new_designation_id, new_division_id)
    dotted_line_name = resolve_designation_name(dotted_line)

    if not user_dept_name or not user_desi_name:
        return 0

    qs = EmployeeWorkDetails.objects.filter(
        IsDelete=False,
        IsSecondary=False,
        EmpStatus__in=["Confirmed", "Not Confirmed", "On Probation"],
        Department=user_dept_name,
        Designation=user_desi_name,
        # OrganizationID=401
    )

    # print("running in thread ------------->")
    # print("new_division_name::",new_division_name)
    # print("new_department_name::",new_department_name)
    # print("new_designation_name::",new_designation_name)
    # print("new_level::",new_level)
    # print("dotted_line_name:",dotted_line_name)
    # print("/end ------------->")
    
    updated_count = qs.update(
        ReportingtoDivision=new_division_name,
        ReportingtoDepartment=new_department_name,
        ReportingtoDesignation=new_designation_name,
        ReportingtoLevel=new_level,
        DottedLine=dotted_line_name
    )
    
    # print("updated_counts here:",updated_count)
...
def run_change_direct_reporting_async(**kwargs):
    Change_Direct_Reporting_To(**kwargs)
...
def OnRollDesignationAdd(request):
    if request.method == "POST":
        ...
        Dotted_Line_Reporting_Designations = request.POST.get('Dotted_Line_Reporting')
        Direct_Reporting_Designations = request.POST.get('Direct_Reporting')
        Direct_Reporting_Division = request.POST.get('Direct_Reporting_Division')
        Direct_Reporting_Department = request.POST.get('Direct_Reporting_Department')
        Direct_Reporting_Level = request.POST.get('Direct_Reporting_Level')
        Update_Employee_Data = request.POST.get('Update_Employee_Data')
        ...
        if designation_id:  
            try:
                ...
                designation.Direct_Reporting_Designation = Direct_Reporting_Designations
                designation.Dotted_Line_Reporting_Designation = Dotted_Line_Reporting_Designations
                designation.Direct_Reporting_Division = Direct_Reporting_Division
                designation.Direct_Reporting_Department = Direct_Reporting_Department
                designation.Direct_Reporting_Level = Direct_Reporting_Level
                designation.save()
            ...
            OnRollDesignationMaster.objects.create(
                ...
                Order=order,
                Direct_Reporting_Designation=Direct_Reporting_Designations,
                Dotted_Line_Reporting_Designation=Dotted_Line_Reporting_Designations,
                Direct_Reporting_Division = Direct_Reporting_Division,
                Direct_Reporting_Department = Direct_Reporting_Department,
                Direct_Reporting_Level = Direct_Reporting_Level
            )
        ...
        # print("Direct_Reporting_Division:",Direct_Reporting_Division)
        # print("Direct_Reporting_Department:",Direct_Reporting_Department)
        # print("Direct_Reporting_Designations:",Direct_Reporting_Designations)
        # print("Direct_Reporting_Level:",Direct_Reporting_Level)
        # print("Dotted_Line_Reporting_Designations:",Dotted_Line_Reporting_Designations)
        # print("Update_Employee_Data:",Update_Employee_Data)

        if Update_Employee_Data in ['1', 'on', True]:
            threading.Thread(
                target=run_change_direct_reporting_async,
                kwargs={
                    "user_department_id": DepartmentID,
                    "user_designation_id": designation_id,
                    "new_division_id": Direct_Reporting_Division,
                    "new_department_id": Direct_Reporting_Department,
                    "new_designation_id": Direct_Reporting_Designations,
                    "new_level": Direct_Reporting_Level,
                    "dotted_line": Dotted_Line_Reporting_Designations,
                },
                daemon=True
            ).start()

            # print("Employee reporting update started in background")
...
#         designation = OnRollDesignationMaster.objects.get(id=designation_id)
#         data = {
#             'id': designation.designations,
#             'DivisionID': designation.OnRollDivisionMaster.DivisionName,
#             'DepartmentID': designation.OnRollDepartmentMaster.DepartmentName,
#             'Div': designation.Div,
...
def designation_details(request):
    ...
    try:
        ...
        data = {
            ...
            'Lavel': designation.Lavel,  
            'Report_Designations': designation.Report_Designations,  
            'Dotted_Line': designation.Dotted_Line_Reporting_Designation,  
            'Direct_Reporting': designation.Direct_Reporting_Designation,  
            'Direct_Division': designation.Direct_Reporting_Division,  
            'Direct_Department': designation.Direct_Reporting_Department,  
            'Direct_Level': designation.Direct_Reporting_Level,  
        }
        return JsonResponse(data)
    except OnRollDesignationMaster.DoesNotExist:
        return JsonResponse({'error': 'Designation not found'}, status=404)
...
def VarianceReportView(request):
    ...
    for budget in managebudget_department_designation:
        ...
        department_totals[dept_id]['total_salary'] += budget.get('aavg_salary', 0) or 0
        department_totals[dept_id]['total_headcount'] += budget.get('head_count', 0) or 0
        
        if department_totals[dept_id]['total_headcount'] > 0:
            department_totals[dept_id]['aavg_salary'] = (
                department_totals[dept_id]['total_salary'] /
                department_totals[dept_id]['total_headcount']
            )
...
def LevelWiseReport(request):
    ...
    if not selected_departments:
        selected_departments = ['All_Department']
    elif 'All_Department' in selected_departments and len(selected_departments) > 1:
        selected_departments.remove('All_Department')

    if not selected_levels:
        selected_levels = ['All_Level']
    elif 'All_Level' in selected_levels and len(selected_levels) > 1:
        selected_levels.remove('All_Level')
...
def variance_details_report_experiment(request):
    ...
    # print("variance_details_report_experiment ------------------")
...
def get_contract_manning_data(selectedOrganizationID):
    # =========================
    # 1️⃣ MASTER STRUCTURE
    # =========================
    contractdivisions = ContractDivisionMaster.objects.filter(
        IsDelete=False
    ).order_by('Order').prefetch_related(
        'contractdepartmentmaster_set__contractdesignationmaster_set'
    )
    ...
    # =========================
    # 2️⃣ ACTUAL CONTRACT DATA
    # =========================
    actual_qs = EntryActualContract.objects.filter(
        hotel_name=selectedOrganizationID
    ).values(
        'contract_department_master__DepartmentName',
        'contract_designation_master__designations'
        ...
        actual_head_count=Sum('head_count'),
        actual_avg_salary=ExpressionWrapper(
    ...
    for row in actual_qs:
        ...
        desig = row['contract_designation_master__designations']

        row['actual_total_ctc'] = row['actual_avg_salary'] * row['actual_head_count']

        actual_dict.setdefault(dept, {})[desig] = row

        Contractdepartment_totals.setdefault(dept, {
            'Contractmultiplication_result': 0,
            'Contracthead_count': 0,
            'salary_headcount_contract': 0
        })

        Contractdepartment_totals[dept]['Contractmultiplication_result'] += row['actual_avg_salary']
        Contractdepartment_totals[dept]['Contracthead_count'] += row['actual_head_count']
        Contractdepartment_totals[dept]['salary_headcount_contract'] += row['actual_total_ctc']

    # =========================
    # 3️⃣ BUDGET CONTRACT DATA
    # =========================
    budget_qs = ManageBudgetContract.objects.filter(
        hotel_name=selectedOrganizationID
    ).values(
        'contract_department_master__DepartmentName',
        'contract_designation_master__designations'
        ...
        budget_head_count=Sum('head_count'),
        budget_avg_salary=ExpressionWrapper(
            Sum('avg_salary') / Count('id'),
            output_field=FloatField()
        )
    )
    ...
    for row in budget_qs:
        ...
        row['budget_total_ctc'] = row['budget_avg_salary'] * row['budget_head_count']

        budget_dict.setdefault(dept, {})[desig] = row

        onrollContractdepartment_totals.setdefault(dept, {
            'Contractmultiplication_result': 0,
            'Contracthead_count': 0,
            'salary_headcount_contract': 0
        })

        onrollContractdepartment_totals[dept]['Contractmultiplication_result'] += row['budget_avg_salary']
        onrollContractdepartment_totals[dept]['Contracthead_count'] += row['budget_head_count']
        onrollContractdepartment_totals[dept]['salary_headcount_contract'] += row['budget_total_ctc']

    # =========================
    # 4️⃣ ROW-LEVEL VARIANCE
    # =========================
    variance_contract_dict = {}

    for dept, designations in actual_dict.items():
        variance_contract_dict.setdefault(dept, {})

        for desig, actual in designations.items():
            budget = budget_dict.get(dept, {}).get(desig, {})

            variance_contract_dict[dept][desig] = {
                'variance_head_count': actual['actual_head_count'] - budget.get('budget_head_count', 0),
                'variance_avg_salary': actual['actual_avg_salary'] - budget.get('budget_avg_salary', 0),
                'variance_total_ctc': actual['actual_total_ctc'] - budget.get('budget_total_ctc', 0),
            }

    # =========================
    # 5️⃣ DEPARTMENT VARIANCE
    # =========================
    onrolldepartment_variance_dict = {}

    for dept in set(Contractdepartment_totals) | set(onrollContractdepartment_totals):
        actual = Contractdepartment_totals.get(dept, {})
        budget = onrollContractdepartment_totals.get(dept, {})

        onrolldepartment_variance_dict[dept] = {
            'variance_multiplication_result': actual.get('Contractmultiplication_result', 0) - budget.get('Contractmultiplication_result', 0),
            'variance_head_count': actual.get('Contracthead_count', 0) - budget.get('Contracthead_count', 0),
            'variance_salary_headcount_contract': actual.get('salary_headcount_contract', 0) - budget.get('salary_headcount_contract', 0),
        }
    ...
    # =========================
    enterycontractdetasgrand = sum(
        d['Contractmultiplication_result'] for d in Contractdepartment_totals.values()
    )

    enterycontractdetasheadgrandtotalon = sum(
        d['Contracthead_count'] for d in Contractdepartment_totals.values()
    )

    enterycontractempgrandsalarydivion = sum(
        d['salary_headcount_contract'] for d in Contractdepartment_totals.values()
    )

    budgetcontractgrandtotalondataemp = sum(
        d['Contractmultiplication_result'] for d in onrollContractdepartment_totals.values()
    )

    budgetcontractgrandtotalondataemp_headcount = sum(
        d['Contracthead_count'] for d in onrollContractdepartment_totals.values()
    )

    budgetcontractdatagrandsalarydivion = sum(
        d['salary_headcount_contract'] for d in onrollContractdepartment_totals.values()
    )
    ...
    return {
        # 'contractdivisions': contractdivisions,
        'contractdivisions': list(
                contractdivisions.values(
                    'id',
                    'DivisionName'
                )
            ),
...
def contract_varience_manning_api(request):
    # if request.method != "GET":
    #     return JsonResponse({'error': 'Method not allowed'}, status=405)

    selectedOrganizationID = request.GET.get('org_id')

    if not selectedOrganizationID:
        return JsonResponse({'error': 'org_id required'}, status=400)

    data = get_contract_manning_data(selectedOrganizationID)
...
# exp --------------------------->
def get_shared_services_manning_data(selectedOrganizationID):
    # =========================
    # MASTER STRUCTURE
    Servicedivisions = ServicesDivisionMaster.objects.filter(
        IsDelete=False
    ).order_by('Order').prefetch_related(
        'servicesdepartmentmaster_set__servicesdesignationmaster_set'
    )
    ...
    # =========================
    # ACTUAL SHARED SERVICES DATA
    actual_qs = EntryActualSharedServices.objects.filter(
        hotel_name=selectedOrganizationID
    ).values(
        'services_department_master__DepartmentName',
        'services_designation_master__designations'
        ...
        actualservice_head_count=Sum('head_count'),
        actualservice_avg_salary=ExpressionWrapper(
            Sum('avg_salary') / Count('id'),
            output_field=FloatField()
        )
    )
    ...
    actualservice_dict = {}
    departmentservices_totals = {}

    for row in actual_qs:
        dept = row['services_department_master__DepartmentName']
        desig = row['services_designation_master__designations']

        # row['actualservice_total_ctc'] = row['actualservice_avg_salary'] * row['actualservice_head_count']
        row['actualservice_total_ctc'] = (row['actualservice_avg_salary'] or 0) * (row['actualservice_head_count'] or 0)

        actualservice_dict.setdefault(dept, {})[desig] = row

        departmentservices_totals.setdefault(dept, {
            'TotalServicesmultiplication_result': 0,
            'TotalServiceshead_count': 0,
            'TotalServicessalary_headcount_contract': 0
        })

        departmentservices_totals[dept]['TotalServicesmultiplication_result'] += row['actualservice_avg_salary'] or 0
        departmentservices_totals[dept]['TotalServiceshead_count'] += row['actualservice_head_count'] or 0
        departmentservices_totals[dept]['TotalServicessalary_headcount_contract'] += row['actualservice_total_ctc'] or 0


    # =========================
    # BUDGET SHARED SERVICES DATA
    budget_qs = ManageBudgetSharedServices.objects.filter(
        hotel_name=selectedOrganizationID
    ).values(
        'services_department_master__DepartmentName',
        'services_designation_master__designations'
        ...
        budgetservice_head_count=Sum('head_count'),
        budgetservice_avg_salary=ExpressionWrapper(
            Sum('avg_salary') / Count('id'),
            output_field=FloatField()
        )
    )
    ...
    budgetservice_dict = {}
    totaldepartmentservices_totals = {}

    for row in budget_qs:
        dept = row['services_department_master__DepartmentName']
        desig = row['services_designation_master__designations']

        # row['budgetservice_total_ctc'] = row['budgetservice_avg_salary'] * row['budgetservice_head_count']
        row['budgetservice_total_ctc'] = (row['budgetservice_avg_salary'] or 0) * (row['budgetservice_head_count'] or 0)
        

        budgetservice_dict.setdefault(dept, {})[desig] = row

        totaldepartmentservices_totals.setdefault(dept, {
            'TotalServicesmultiplication_result': 0,
            'TotalServiceshead_count': 0,
            'TotalServicessalary_headcount_contract': 0
        })
        
        totaldepartmentservices_totals[dept]['TotalServicesmultiplication_result'] += row['budgetservice_avg_salary'] or 0
        totaldepartmentservices_totals[dept]['TotalServiceshead_count'] += row['budgetservice_head_count'] or 0
        totaldepartmentservices_totals[dept]['TotalServicessalary_headcount_contract'] += row['budgetservice_total_ctc'] or 0
    ...
    for dept, designations in actualservice_dict.items():
        variance_shared_services_dict.setdefault(dept, {})
        for desig, actual in designations.items():
            budget = budgetservice_dict.get(dept, {}).get(desig, {})
            variance_shared_services_dict[dept][desig] = {
                'variance_head_count': (actual['actualservice_head_count'] or 0) - (budget.get('budgetservice_head_count') or 0),
                'variance_avg_salary': (actual['actualservice_avg_salary'] or 0) - (budget.get('budgetservice_avg_salary') or 0),
                'variance_total_ctc': (actual['actualservice_total_ctc'] or 0) - (budget.get('budgetservice_total_ctc') or 0),
            }

    # =========================
    # DEPARTMENT TOTAL VARIANCE
    onrolldepartment_variance_dictss = {}
    for dept in set(departmentservices_totals) | set(totaldepartmentservices_totals):
        actual = departmentservices_totals.get(dept, {})
        budget = totaldepartmentservices_totals.get(dept, {})
        onrolldepartment_variance_dictss[dept] = {
            'variance_multiplication_result': actual.get('TotalServicesmultiplication_result', 0) - budget.get('TotalServicesmultiplication_result', 0),
            'variance_head_count': actual.get('TotalServiceshead_count', 0) - budget.get('TotalServiceshead_count', 0),
            'variance_salary_headcount_contract': actual.get('TotalServicessalary_headcount_contract', 0) - budget.get('TotalServicessalary_headcount_contract', 0)
        }

    # =========================
    # GRAND TOTALS
    grand_actual_multiplication = sum(d['TotalServicesmultiplication_result'] for d in departmentservices_totals.values())
    grand_actual_headcount = sum(d['TotalServiceshead_count'] for d in departmentservices_totals.values())
    grand_actual_total_ctc = sum(d['TotalServicessalary_headcount_contract'] for d in departmentservices_totals.values())

    grand_budget_multiplication = sum(d['TotalServicesmultiplication_result'] for d in totaldepartmentservices_totals.values())
    grand_budget_headcount = sum(d['TotalServiceshead_count'] for d in totaldepartmentservices_totals.values())
    grand_budget_total_ctc = sum(d['TotalServicessalary_headcount_contract'] for d in totaldepartmentservices_totals.values())

    grand_variance_multiplication = grand_actual_multiplication - grand_budget_multiplication
    grand_variance_headcount = grand_actual_headcount - grand_budget_headcount
    grand_variance_total_ctc = grand_actual_total_ctc - grand_budget_total_ctc

    # =========================
    # RETURN EVERYTHING
    return {
        'Servicedivisions': list(Servicedivisions.values('id', 'DivisionName')),
        'actualservice_dict': actualservice_dict,
        'budgetservice_dict': budgetservice_dict,
        'variance_shared_services_dict': variance_shared_services_dict,
        'onrolldepartment_variance_dictss': onrolldepartment_variance_dictss,
        'grand_actual_multiplication': grand_actual_multiplication,
        'grand_actual_headcount': grand_actual_headcount,
        'grand_actual_total_ctc': grand_actual_total_ctc,
        'grand_budget_multiplication': grand_budget_multiplication,
        'grand_budget_headcount': grand_budget_headcount,
        'grand_budget_total_ctc': grand_budget_total_ctc,
        'grand_variance_multiplication': grand_variance_multiplication,
        'grand_variance_headcount': grand_variance_headcount,
        'grand_variance_total_ctc': grand_variance_total_ctc,
    }



def shared_services_manning_api(request):
    selectedOrganizationID = request.GET.get('org_id')

    if not selectedOrganizationID:
        return JsonResponse({'error': 'org_id required'}, status=400)

    data = get_shared_services_manning_data(selectedOrganizationID)
...
# @require_GET
# def budget_onroll_variance_totals_api(request):
#     hotel_name = request.GET.get('OID')  # example: 401

#     if not hotel_name:
#         return JsonResponse({
#             "status": False,
#             "message": "hotel_name is required"
#         }, status=400)
...
#     if hotel_name:
#         employees_per_department_designation = employees_per_department_designation.filter(OrganizationID=hotel_name,IsDelete=False,IsSecondary=False,EmpStatus__in=["On Probation", "Confirmed", "Not Confirmed"])
...
#     qs = ManageBudgetOnRoll.objects.filter(
#         hotel_name=hotel_name,
#         is_delete=False
#     )

#     totals = qs.aggregate(
#         total_avg_salary=Sum('avg_salary'),
#         total_head_count=Sum('head_count'),
#         total_ctc_count=Sum('total_ctc')
#     )

#     total_avg_salary = totals['total_avg_salary'] or 0
#     total_head_count = totals['total_head_count'] or 0
#     total_ctc_count = totals['total_ctc_count'] or 0

#     total_budget_avg_salary = (
#         total_ctc_count / total_head_count
#         if total_head_count > 0 else 0
#     )

#     # total_budget_avg_salary = (
#     #     total_avg_salary / total_head_count
#     #     if total_head_count > 0 else 0
#     # )
...
#                 "total_ctc_count": total_ctc_count,
#                 "total_budget_avg_salary": round(total_budget_avg_salary, 2)
#             }
#         }
...
def budget_onroll_variance_totals_api(request):
    ...
    # ==============================
    employees_qs = EmployeeWorkDetails.objects.filter(
        OrganizationID=hotel_name,
        IsDelete=False,
        IsSecondary=False,
        EmpStatus__in=["On Probation", "Confirmed", "Not Confirmed"]
    )

    employee_totals = employees_qs.aggregate(
        total_headcount=Count('id'),
        total_salary=Sum('Salary')
    )

    total_actual_headcount = employee_totals['total_headcount'] or 0
    total_actual_salary = employee_totals['total_salary'] or Decimal(0)

    total_actual_avg_salary = (
        Decimal(total_actual_salary) / Decimal(total_actual_headcount)
        if total_actual_headcount > 0 else Decimal(0)
    )
    ...
    budget_totals = budget_qs.aggregate(
        total_budget_head_count=Sum('head_count'),
        total_budget_ctc=Sum('total_ctc')
    )

    total_budget_head_count = budget_totals['total_budget_head_count'] or 0
    total_budget_ctc = budget_totals['total_budget_ctc'] or Decimal(0)

    total_budget_avg_salary = (
        Decimal(total_budget_ctc) / Decimal(total_budget_head_count)
        if total_budget_head_count > 0 else Decimal(0)
    )

    # VARIANCE
    variance_headcount = total_budget_head_count - total_actual_headcount
    variance_total_salary = total_budget_ctc - total_actual_salary
    variance_avg_salary = total_budget_avg_salary - total_actual_avg_salary

    # ==============================
    # BUDGET Contract DATA
    # ==============================
    budget_Contract_qs = ManageBudgetContract.objects.filter(
        hotel_name=hotel_name,
        IsDelete=False
    )

    budget_Contract_totals = budget_Contract_qs.aggregate(
        total_budget_Contract_head_count=Sum('head_count'),
        total_budget_Contract_ctc=Sum('total_ctc')
    )

    total_budget_Contract_head_count = budget_Contract_totals['total_budget_Contract_head_count'] or 0
    total_budget_Contract_ctc = budget_Contract_totals['total_budget_Contract_ctc'] or Decimal(0)

    total_budget_Contract_avg_salary = (
        Decimal(total_budget_Contract_ctc) / Decimal(total_budget_Contract_head_count)
        if total_budget_Contract_head_count > 0 else Decimal(0)
    )
    
    # Actual Contract 
    Actual_Contract_qs = EntryActualContract.objects.filter(
        hotel_name=hotel_name,
        IsDelete=False
    )

    Actual_Contract_totals = Actual_Contract_qs.aggregate(
        total_Actual_Contract_head_count=Sum('head_count'),
        total_Actual_Contract_ctc=Sum('total_ctc')
    )

    total_Actual_Contract_head_count = Actual_Contract_totals['total_Actual_Contract_head_count'] or 0
    total_Actual_Contract_ctc = Actual_Contract_totals['total_Actual_Contract_ctc'] or Decimal(0)

    total_Actual_Contract_avg_salary = (
        Decimal(total_Actual_Contract_ctc) / Decimal(total_Actual_Contract_head_count)
        if total_Actual_Contract_head_count > 0 else Decimal(0)
    )

    # VARIANCE
    variance_Contract_headcount = total_budget_Contract_head_count - total_Actual_Contract_head_count
    variance_Contract_total_salary = total_budget_Contract_ctc - total_Actual_Contract_ctc
    variance_Contract_avg_salary = total_budget_Contract_avg_salary - total_Actual_Contract_avg_salary

    # ==============================
    # BUDGET SHARED SERVICES DATA
    # ==============================
    budget_shared_services_qs = ManageBudgetSharedServices.objects.filter(
        hotel_name=hotel_name,
        IsDelete=False
    )

    budget_shared_services_totals = budget_shared_services_qs.aggregate(
        total_budget_shared_head_count=Sum('head_count'),
        total_budget_shared_ctc=Sum('total_ctc')
    )

    total_budget_shared_head_count = (
        budget_shared_services_totals['total_budget_shared_head_count'] or 0
    )
    total_budget_shared_ctc = (
        budget_shared_services_totals['total_budget_shared_ctc'] or Decimal(0)
    )

    total_budget_shared_avg_salary = (
        Decimal(total_budget_shared_ctc) / Decimal(total_budget_shared_head_count)
        if total_budget_shared_head_count > 0 else Decimal(0)
    )

    # ACTUAL SHARED SERVICES DATA
    actual_shared_services_qs = EntryActualSharedServices.objects.filter(
        hotel_name=hotel_name,
        IsDelete=False
    )

    actual_shared_services_totals = actual_shared_services_qs.aggregate(
        total_actual_shared_head_count=Sum('head_count'),
        total_actual_shared_ctc=Sum('total_ctc')
    )

    total_actual_shared_head_count = (
        actual_shared_services_totals['total_actual_shared_head_count'] or 0
    )
    total_actual_shared_ctc = (
        actual_shared_services_totals['total_actual_shared_ctc'] or Decimal(0)
    )

    total_actual_shared_avg_salary = (
        Decimal(total_actual_shared_ctc) / Decimal(total_actual_shared_head_count)
        if total_actual_shared_head_count > 0 else Decimal(0)
    )


    # ==============================
    # VARIANCE SHARED SERVICES
    # ==============================
    variance_shared_headcount = (
        total_budget_shared_head_count - total_actual_shared_head_count
    )

    variance_shared_total_salary = (
        total_budget_shared_ctc - total_actual_shared_ctc
    )

    variance_shared_avg_salary = (
        total_budget_shared_avg_salary - total_actual_shared_avg_salary
    )

    # # ==============================
    # # BUDGET MEAL COST
    # budget_meal_qs = BudgetMealCost.objects.filter(
    #     hotel_name=hotel_name,
    #     IsDelete=False
    # )

    # budget_meal_totals = budget_meal_qs.aggregate(
    #     total_budget_meal_cost=Sum('cafeteriamealcost')
    # )

    # total_budget_meal_cost = (
    #     budget_meal_totals['total_budget_meal_cost'] or Decimal(0)
    # )


    # # ==============================
    # # ACTUAL MEAL COST
    # actual_meal_qs = EntryActualMealCost.objects.filter(
    #     hotel_name=hotel_name,
    #     IsDelete=False
    # )

    # actual_meal_totals = actual_meal_qs.aggregate(
    #     total_actual_meal_cost=Sum('cafeteriamealcost')
    # )

    # total_actual_meal_cost = (
    #     actual_meal_totals['total_actual_meal_cost'] or Decimal(0)
    # )

    # # VARIANCE MEAL COST
    # variance_meal_cost = total_budget_meal_cost - total_actual_meal_cost
    ...
    budget_meal_obj = budget_meal_qs.first()

    total_budget_meal_cost = (
        budget_meal_obj.cafeteriamealcost
        if budget_meal_obj else Decimal(0)
    )
    ...
    actual_meal_obj = actual_meal_qs.first()

    total_actual_meal_cost = (
        actual_meal_obj.cafeteriamealcost
        if actual_meal_obj else Decimal(0)
    )
    ...
    # ==============================
    variance_meal_cost = (
        total_budget_meal_cost - total_actual_meal_cost
    )


    # ==============================
    # BUDGET INSURANCE COST
    budget_insurance_obj = BudgetInsuranceCost.objects.filter(
        hotel_name=hotel_name,
        IsDelete=False
    ).first()

    total_budget_insurance_cost = (
        budget_insurance_obj.EmployeeInsurancecost
        if budget_insurance_obj else Decimal(0)
    )

    actual_insurance_obj = EntryActualInsuranceCost.objects.filter(
        hotel_name=hotel_name,
        IsDelete=False
    ).first()

    total_actual_insurance_cost = (
        actual_insurance_obj.EmployeeInsurancecost
        if actual_insurance_obj else Decimal(0)
    )

    variance_insurance_cost = (
        total_budget_insurance_cost - total_actual_insurance_cost
    )
    ...
    return JsonResponse({
        ...
        "data": {
            "On_Roll": {
                "Budget_On_Roll": {
                    "headcount": total_budget_head_count,
                    "total_salary": float(total_budget_ctc),
                    "avg_salary": round(float(total_budget_avg_salary), 2)
                },
                "Actual_On_Roll": {
                    "headcount": total_actual_headcount,
                    "total_salary": float(total_actual_salary),
                    "avg_salary": round(float(total_actual_avg_salary), 2)
                },
                "Variance": {
                    "headcount": variance_headcount,
                    "total_salary": round(float(variance_total_salary), 2),
                    "avg_salary": round(float(variance_avg_salary), 2)
                }
            },
            "Total_Contract": {
                "Budget_On_Roll": {
                    "headcount": total_budget_Contract_head_count,
                    "total_salary": float(total_budget_Contract_ctc),
                    "avg_salary": round(float(total_budget_Contract_avg_salary), 2)
                },
                "Actual_On_Roll": {
                    "headcount": total_Actual_Contract_head_count,
                    "total_salary": float(total_Actual_Contract_ctc),
                    "avg_salary": round(float(total_Actual_Contract_avg_salary), 2)
                },
                "Variance": {
                    "headcount": variance_Contract_headcount,
                    "total_salary": round(float(variance_Contract_total_salary), 2),
                    "avg_salary": round(float(variance_Contract_avg_salary), 2)
                }
            },
            "Total_Shared_Services": {
                "Budget_On_Roll": {
                    "headcount": total_budget_shared_head_count,
                    "total_salary": float(total_budget_shared_ctc),
                    "avg_salary": round(float(total_budget_shared_avg_salary), 2)
                },
                "Actual_On_Roll": {
                    "headcount": total_actual_shared_head_count,
                    "total_salary": float(total_actual_shared_ctc),
                    "avg_salary": round(float(total_actual_shared_avg_salary), 2)
                },
                "Variance": {
                    "headcount": variance_shared_headcount,
                    "total_salary": round(float(variance_shared_total_salary), 2),
                    "avg_salary": round(float(variance_shared_avg_salary), 2)
                }
            },
            "Total_Meal_Cost": {
                "Bud_Total_CTC": float(total_budget_meal_cost),
                "Act_Total_CTC": float(total_actual_meal_cost),
                "Var_Total_CTC": round(float(variance_meal_cost), 2)
            },
            "Total_Insurance_Cost": {
                "Bud_Total_CTC": float(total_budget_insurance_cost),
                "Act_Total_CTC": float(total_actual_insurance_cost),
                "Var_Total_CTC": round(float(variance_insurance_cost), 2)
            }
        }
    })
