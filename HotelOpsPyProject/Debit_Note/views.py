...
from .models import Debit_Note_Employee_Detail,Debit_Note_Deleted_File_Of_Employee
from django.shortcuts import render,redirect
...
def Debit_Note_Entry_Emp(request):
    ...
    if EmpCode is not None:
        if LOE_ID is not None:
            Experienceobj = Debit_Note_Employee_Detail.objects.filter(id=LOE_ID, emp_code=EmpCode, OrganizationID=OrganizationID, IsDelete=False).first()
        else:
            Experienceobj = Debit_Note_Employee_Detail.objects.filter(emp_code=EmpCode, OrganizationID=OrganizationID, IsDelete=False).first()
    ...
    if request.method == "POST":
        ...
        if DataFromExperienceobj == "Experienceobj" and LOE_ID:
            ...
            Experienceobj = Debit_Note_Employee_Detail.objects.create(
                emp_code=emp_code,
                prefix=prefix,
        ...
        if Page == 'Debit_Note_Emp_List':
            return redirect('Debit_Note_Emp_List')
    ...
    return render(request, "Debit_Note/Entry_Emp.html", context)



from django.db.models import Subquery, OuterRef
from app.views import OrganizationList
def Debit_Note_Emp_List(request):
    ...
    ExperienceLetters = Debit_Note_Employee_Detail.objects.filter(IsDelete=False,OrganizationID= I).annotate(
    ...
    return render(request,"Debit_Note/Emp_ListPage.html",context)

# For generate_appointment_letter
def Generate_Letter_of_Debit_Note(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)

    id = request.GET.get("LOE_ID")
    OID = request.GET.get("OID")
    emp = Debit_Note_Employee_Detail.objects.get(id=id)
    policy = Letter_Editor.objects.filter(OrganizationID=OID, IsDelete=False, Letter_Data_Item_Master_id=6).first()
    ...
    template = get_template("Debit_Note/Letter_Of_Debit_Note.html")
...
def Debit_Note_Emp_Delete(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    EmpID = request.GET.get('EmpID')
    id = request.GET.get('LOE_ID') 
    empdetail = Debit_Note_Employee_Detail.objects.get(id=id)
...
def Debit_Note_Upload_File(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    return render(request, "Debit_Note/Upload_File.html")


def Debit_Note_Download_File(request):
    id = request.GET.get('LOE_ID')
    OID  = request.GET.get('OID')
    if OID:
            OrganizationID= OID   
    file = Debit_Note_Employee_Detail.objects.get(pk=id)
...
def Debit_Note_Repalce_File(request):
    if 'OrganizationID' not in request.session:
            return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    file_id = file.file_id
    file_name = file.file_name
    EmployeeDetail = Debit_Note_Employee_Detail.objects.get(pk=id)
    
    
    deletefile = Debit_Note_Deleted_File_Of_Employee.objects.create(Debit_Note_Employee_Detail= EmployeeDetail,file_id = file_id,file_name = file_name)
