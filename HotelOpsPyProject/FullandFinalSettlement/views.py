...
def FullAndFinal_Approval_List(request): 
    ...
    if not OID:  # handles '', None
        OID = OrganizationID
    else:
        OID = int(OID)
        
    FinalStatus = request.GET.get("FinalStatus", "Pending")
    PaymentStatus = request.GET.get("PaymentStatus", "Pending")
    Approval_Stage = request.GET.get("Approval_Stage", '').lower()
    ...
    if OID and OID != "All":
        fanfs = fanfs.filter(OrganizationID=OID)        
        
    if Dept != '':
        fanfs = fanfs.filter(Dept=Dept)
        
    if PaymentStatus:
        fanfs = fanfs.filter(PaymentStatus=PaymentStatus)
        
    if FinalStatus and FinalStatus.lower() != "all":
        fanfs = fanfs.filter(FinalStatus=FinalStatus)
        
    if Approval_Stage and Approval_Stage == 'hr':
        fanfs = fanfs.filter(AuditedByHR=Approval_Status)
        
    if Approval_Stage and Approval_Stage == 'fc':
        fanfs = fanfs.filter(AuditedByFinance=Approval_Status)
        
    if Approval_Stage and Approval_Stage == 'gm':
        fanfs = fanfs.filter(AuditedByGM=Approval_Status)
        
    if Approval_Stage and Approval_Stage == 'auditor':
        fanfs = fanfs.filter(AuditedBy_Auditor=Approval_Status)
    ...
    for obj in fanfs:
        obj.EmpID_Resolved = emp_map.get(obj.Emp_Code, None)
        obj.HTL = org["ShortDisplayLabel"] if org else None
...
#         for item in data:
#             org_map = {
#                 org["OrganizationID"]: org["ShortDisplayLabel"]
#                 for org in OrganizationMaster.objects.filter(
#                     IsDelete=False,
#                     IsNileHotel=1,
#                     Activation_status=1
...
#             for item in data:
#                 item["HTL"] = org_map.get(item.get("OrganizationID"))
...
#     if PaymentStatus:
#         qs = qs.filter(PaymentStatus=PaymentStatus)
        
#     if FinalStatus and FinalStatus.lower() != "all":
#         qs = qs.filter(FinalStatus=FinalStatus)
        
#     if Approval_Stage and Approval_Stage == 'hr':
#         qs = qs.filter(AuditedByHR=Approval_Status)
        
#     if Approval_Stage and Approval_Stage == 'fc':
#         qs = qs.filter(AuditedByFinance=Approval_Status)
        
#     if Approval_Stage and Approval_Stage == 'gm':
#         qs = qs.filter(AuditedByGM=Approval_Status)
        
#     if Approval_Stage and Approval_Stage == 'auditor':
#         qs = qs.filter(AuditedBy_Auditor=Approval_Status)
        
#     # print("Approval_Stage:",Approval_Stage)
...
def get_fandf_by_OID(request):
    Fixed_Token = (
        'ujhj45ON8BKl!udLGPu!szcWtY!e9MTm4jpXSqD7wNM1HITpnbJhhp='
        'aElxgkShcdaBhvgqLeOMjz9G?qliY6FK/AcJN0iTB3fIl5g55bllJHdrF-'
        'Yh-O4W-eEjKaPk/DBGqHU6XDhbG5m68RtVxZGH?B6n1F5u=F84npBeJI'
        'MS/SzrT7=dXuAj=8aqDyvRpIh=nswd!XPTMobzhw2jKxocrOYJkzo0'
        'osZFSMxK1hMqRbqGJIKR=bgRfS!cea11f'
    )
    ...
    if OID != '333333' and not OrganizationMaster.objects.filter(
        OrganizationID=OID
    ).exists():
        return Response({'error': 'Invalid OrganizationID'}, status=400)
    ...
    # Case 1: stage = all AND status = all
    if Approval_Stage == "all" and Approval_Status == "all":
        print("Stage = all AND Status = all → FinalStatus = Pending only")

        qs = qs.filter(FinalStatus="Pending")


    # Case 2: stage = all AND status = Approved / Pending
    elif Approval_Stage == "all":
        print("Stage = all AND specific status → AND condition on all stages")

        status = Approval_Status or "Pending"

        qs = qs.filter(
            AuditedByHR=status,
            AuditedByFinance=status,
            AuditedByGM=status,
            AuditedBy_Auditor=status
        )


    # Case 3: specific stage AND status = all
    elif Approval_Status == "all":
        print(f"Stage = {Approval_Stage} AND Status = all → no status filter")
        
        pass


    # Case 4: specific stage AND specific status
    elif Approval_Stage == "hr":
        qs = qs.filter(AuditedByHR=Approval_Status)

    elif Approval_Stage == "fc":
        qs = qs.filter(AuditedByFinance=Approval_Status)

    elif Approval_Stage == "gm":
        qs = qs.filter(AuditedByGM=Approval_Status)

    elif Approval_Stage == "auditor":
        qs = qs.filter(AuditedBy_Auditor=Approval_Status)
    ...
    # ---------------- ORDER ----------------
    qs = qs.order_by("-Date_Of_Leaving")
