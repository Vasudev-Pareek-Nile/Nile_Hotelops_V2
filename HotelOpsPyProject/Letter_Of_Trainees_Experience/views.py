...
from .models import Letter_Of_Trainees_Experience,Trainees_Experience_Employee_Detail,Trainees_Experience_Deleted_File_Of_Employee
from django.shortcuts import render,redirect
...
def Trainees_Entry_Emp(request):
    ...
    if EmpCode is not None:
        if LOE_ID is not None:
            Experienceobj = Trainees_Experience_Employee_Detail.objects.filter(id=LOE_ID, emp_code=EmpCode, OrganizationID=OrganizationID, IsDelete=False).first()
        else:
            Experienceobj = Trainees_Experience_Employee_Detail.objects.filter(emp_code=EmpCode, OrganizationID=OrganizationID, IsDelete=False).first()
    ...
    if request.method == "POST":
        ...
        if DataFromExperienceobj == "Experienceobj" and LOE_ID:
            ...
            Experienceobj = Trainees_Experience_Employee_Detail.objects.create(
                emp_code=emp_code,
                prefix=prefix,
    ...
    return render(request, "Trainees_Exp/Entry_Emp.html", context)



from django.db.models import Subquery, OuterRef
from app.views import OrganizationList
def Trainees_Emp_List(request):
    ...
    ExperienceLetters = Trainees_Experience_Employee_Detail.objects.filter(IsDelete=False,OrganizationID= I).annotate(
    ...
    return render(request,"Trainees_Exp/Emp_ListPage.html",context)

# For generate_appointment_letter
def Generate_Letter_of_Trainees_Experience(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)

    id = request.GET.get("LOE_ID")
    emp = Trainees_Experience_Employee_Detail.objects.get(id=id)
    ...
    template = get_template("Trainees_Exp/Letter_Of_Exp_View.html")
...
def Trainees_Emp_Delete(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    EmpID = request.GET.get('EmpID')
    id = request.GET.get('LOE_ID') 
    empdetail = Trainees_Experience_Employee_Detail.objects.get(id=id)
...
def Trainees_Upload_File(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
...
def Trainees_Download_File(request):
    id = request.GET.get('LOE_ID')
    OID  = request.GET.get('OID')
    if OID:
            OrganizationID= OID   
    file = Trainees_Experience_Employee_Detail.objects.get(pk=id)
...
def Trainees_Repalce_File(request):
    if 'OrganizationID' not in request.session:
            return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    file_id = file.file_id
    file_name = file.file_name
    EmployeeDetail = Trainees_Experience_Employee_Detail.objects.get(pk=id)
    
    
    deletefile = Trainees_Experience_Deleted_File_Of_Employee.objects.create(Trainees_Experience_Employee_Detail= EmployeeDetail,file_id = file_id,file_name = file_name)
