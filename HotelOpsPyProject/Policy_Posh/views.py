...
from .models import Policy_Posh_Employee_Detail,Policy_Posh_Deleted_File_Of_Employee
from django.shortcuts import render,redirect
from xhtml2pdf import pisa
from django.template.loader import get_template


from HumanResources.views import EmployeeDetailsData,HrNameOnTheBasisofDesignation,EmployeeNameOnTheBasisofDesignation
...
def Policy_Posh_Entry_Emp(request):
    ...
    if EmpCode is not None:
        if LOE_ID is not None:
            Experienceobj = Policy_Posh_Employee_Detail.objects.filter(id=LOE_ID, emp_code=EmpCode, OrganizationID=OrganizationID, IsDelete=False).first()
        else:
            Experienceobj = Policy_Posh_Employee_Detail.objects.filter(emp_code=EmpCode, OrganizationID=OrganizationID, IsDelete=False).first()
    ...
    if request.method == "POST":
        ...
        if DataFromExperienceobj == "Experienceobj" and LOE_ID:
            ...
            Experienceobj = Policy_Posh_Employee_Detail.objects.create(
                emp_code=emp_code,
                prefix=prefix,
        ...
        if Page == 'Policy_Posh_Emp_List':
            return redirect('Policy_Posh_Emp_List')
    ...
    return render(request, "Policy_Posh/Entry_Emp.html", context)



from django.db.models import Subquery, OuterRef
from app.views import OrganizationList
def Policy_Posh_Emp_List(request):
    ...
    ExperienceLetters = Policy_Posh_Employee_Detail.objects.filter(IsDelete=False,OrganizationID= I).annotate(
    ...
    return render(request,"Policy_Posh/Emp_ListPage.html",context)

# For generate_appointment_letter
def Generate_Letter_of_Policy_Posh(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)

    id = request.GET.get("LOE_ID")
    OID = request.GET.get("OID")
    emp = Policy_Posh_Employee_Detail.objects.get(id=id)
    policy = Letter_Editor.objects.filter(OrganizationID=OID, IsDelete=False, Letter_Data_Item_Master_id=1).first()
    ...
    template = get_template("Policy_Posh/Letter_Of_Policy_Posh.html")
...
def Policy_Posh_Emp_Delete(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    EmpID = request.GET.get('EmpID')
    id = request.GET.get('LOE_ID') 
    empdetail = Policy_Posh_Employee_Detail.objects.get(id=id)
...
def Policy_Posh_Upload_File(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    return render(request, "Policy_Posh/Upload_File.html")


def Policy_Posh_Download_File(request):
    id = request.GET.get('LOE_ID')
    OID  = request.GET.get('OID')
    if OID:
            OrganizationID= OID   
    file = Policy_Posh_Employee_Detail.objects.get(pk=id)
...
def Policy_Posh_Repalce_File(request):
    if 'OrganizationID' not in request.session:
            return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    file_id = file.file_id
    file_name = file.file_name
    EmployeeDetail = Policy_Posh_Employee_Detail.objects.get(pk=id)
    
    
    deletefile = Policy_Posh_Deleted_File_Of_Employee.objects.create(Policy_Posh_Employee_Detail= EmployeeDetail,file_id = file_id,file_name = file_name)
