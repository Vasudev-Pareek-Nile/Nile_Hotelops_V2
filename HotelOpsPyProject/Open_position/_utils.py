...
def generate_position_image(position_name, qr_url, location_name):
    imgname = OrganizationMaster.objects.filter(OrganizationID=location_name).values('Career_Share_Logo').first()
    if imgname and imgname['Career_Share_Logo']:
        imgname = imgname['Career_Share_Logo']
    base_image_path_without_ext = os.path.join(settings.MEDIA_ROOT, 'Hotel_Logos', imgname)
    ...
    if os.path.exists(f"{base_image_path_without_ext}.jpeg"):
        ...
        base_image_path = f"{base_image_path_without_ext}.jpg"
    elif os.path.exists(f"{base_image_path_without_ext}"):
        base_image_path = f"{base_image_path_without_ext}"
    elif os.path.exists(f"{base_image_path_without_ext}"):
        base_image_path = f"{base_image_path_without_ext}"
    else:
    ...
    if current_location in selected_orgs:
        ...
        # hiring_center_x = 2780   # Left (decrease the value) - Right (increase the value)
        hiring_center_x = 2900   # Left (decrease the value) - Right (increase the value)
        hiring_bottom_y = 1000   # upward: (decrease the value) -- downward: (increase the value)

        # Center align the position name under the heading
    ...
    draw.text((scan_text_x, scan_text_y), scan_text, font=font_small, fill=color) 
    # draw.text((scan_text_x, scan_text_y), scan_text, font=font_small, fill=color)  
