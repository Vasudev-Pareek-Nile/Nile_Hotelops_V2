...
from Manning_Guide.models import EntryActualContract,EntryActualSharedServices,EntryActualMealCost,ManageBudgetSharedServices,BudgetInsuranceCost,EntryActualInsuranceCost,BudgetMealCost,ManageBudgetContract, ServicesDepartmentMaster,ServicesDesignationMaster,ContractDesignationMaster,ContractDepartmentMaster, ManageBudgetOnRoll
from HumanResources.models import EmployeePersonalDetails,EmployeeWorkDetails,EmployeeEmergencyInformationDetails, EmployeeAddressInformationDetails,Salary_Detail_Master, EmployeeIdentityInformationDetails, EmployeeBankInformationDetails
from Manning_Guide.models import OnRollDesignationMaster,OnRollDepartmentMaster,LavelAdd, OnRollDivisionMaster  
from django.db.models import OuterRef, Subquery, F, Exists, Value, Count, DecimalField, Case, When, CharField
from Leave_Management_System.models import Emp_Leave_Balance_Master, Leave_Type_Master,Leave_Application
...
from app.models import OrganizationMaster, ReportModuleMaster, City_Location_Master
...
# import pandas as pd
# import pandas as pd
# from Manning_Guide.models import ServicesDepartmentMaster,ServicesDesignationMaster,ContractDesignationMaster,ContractDepartmentMaster, ManageBudgetOnRoll
...
if user_id == [None]:
    ...
    if user_id:
        ...
        if response.status_code == 200:
            ...
            """
            # st.markdown(f"""    
            # <div data-testid="stLogoSpacer" style="text-align:center; font-weight:bold;">
            #     {full_name}
            # </div>
            # """, unsafe_allow_html=True)

            full_name = f"{full_name}"  # Your candidate name
            ...
            # st.markdown("""
            # <style>
            # /* Remove label space above all selectboxes */
            ...
            st.markdown("""
            <style>
            /* Hide label only for selectbox with key=org_selectbox */
            div[data-testid="stSelectbox"]:has(input[aria-label*="Selected"])
                ~ label {
                display: none !important;
            }

            /* Better & safer selector */
            div[data-testid="stSelectbox"]:has(~ input[key="org_selectbox"])
                label {
                display: none !important;
            }
            </style>
            ...
            if org_id is None and org_id == 'All':
...
if 'organization_id' not in st.session_state:
    # print("i am here at error page:, organization id is not showing in session")
...
selected_module = st.sidebar.selectbox(
    "Choose a Module",
    ["Choose a Module"] + module_names,
    label_visibility="collapsed"
)

if selected_module == "Choose a Module":
    selected_module = "Human Resources"  # or "All"
...
selected_report = st.sidebar.selectbox(
    "Choose a report type",
    ["Choose a report type"] + report_options,
    label_visibility="collapsed"
)

if selected_report == "Choose a report type":
    selected_report = "Master Report"  # or "All"
...
if selected_report == "Actual Shared Services":
    ...
    selected_department = st.sidebar.selectbox(
        "Choose a Department", 
        ['Choose a Department'] + list(departments),
        label_visibility="collapsed"
    )   

    if selected_department == "Choose a Department":
    ...
    # selected_designation = st.sidebar.selectbox("Choose a Designation", ['Choose a Designation'] + list(filtered_designations))
    
    selected_designation = st.sidebar.selectbox(
        "Choose a Designation", 
        ['Choose a Designation'] + list(filtered_designations),
        label_visibility="collapsed"
    )   
    if selected_designation == "Choose a Designation":
        selected_designation = "All"
    ...
    selected_Division = st.sidebar.selectbox(
        "Choose a Division", 
        ['Choose a Division'] + list(Division),
        label_visibility="collapsed"
    )   

    if selected_Division == "Choose a Division":
...
# OnRollDepartmentMaster>OnRollDivisionMaster(foreign key) > DivisionName
    ...
    selected_department = st.sidebar.selectbox(
        "Choose a Department", 
        ['Choose a Department'] + list(filtered_departments),
        label_visibility="collapsed"
    )   
...
levels = list(levels)  
levels.insert(0, 'Choose a level')  
...
years = [str(year) for year in range(current_year + 1, 2017, -1)]
...
def fetch_Locker_Allotment_data(report_type, report_options, org_id, selected_Division, selected_department, selected_designation,selected_year, selected_months,):
...
def fetch_Birthdays_data(report_type, report_options,selected_Division, org_id, selected_year, selected_months, selected_department, selected_designation):
    # print("selected_year is here::", selected_year)
    # print("selected_months is here::", selected_months)
    ...
    if report_type == "Upcoming Birthday's report":
        ...
        # employees = employees.filter(DateofBirth__month=current_month)
        # if not selected_months:
        #     employees = employees.filter(DateofBirth__month=current_month)

        if selected_months:
            employees = employees.filter(
                DateofBirth__month__in=[
                    months.index(month) + 1 for month in selected_months
                ]
            )
        else:
            current_month = timezone.now().month
...
def fetch_leave_consumed_report(report_type, report_options, org_id, selected_department, selected_designation):
    if report_type not in report_options:
        raise ValueError(f"Invalid report type: '{report_type}'. Please select a valid report from: {report_options}")
    
    if report_type == "Leave Consumed Report":
        # Query work details and leave balances
        if org_id is None:
            work_details = EmployeeWorkDetails.objects.filter(
                EmpID=OuterRef('EmpID'),
...
def fetch_leave_consumed_report(report_type, report_options, org_id, selected_Division, selected_department, selected_designation, selected_year, selected_months):
    if report_type not in report_options:
        raise ValueError(
            f"Invalid report type: '{report_type}'. Please select a valid report from: {report_options}"
        )

    if report_type == "Leave Consumed Report":
        # print("year:", selected_year)
        # print("months:", selected_months)
        # EmpCode = st.sidebar.text_input("Employee Code", "").strip()

        EmpCode = st.sidebar.text_input(
            "Enter Employee Code",
        ...
        leave_type_dict = {str(lt.id): lt.Type for lt in leave_types}  # id -> type mapping

        # Show multi-select in sidebar
        selected_leave_ids = st.sidebar.multiselect(
            "Choose Leaves",
            options=list(leave_type_dict.keys()),  # use IDs as options
            format_func=lambda x: leave_type_dict[x],  # show the Type name in dropdown
            default=list(leave_type_dict.keys())  # select all by default
        )
        ...
        # Apply filters        
        if EmpCode != '':
            # employees = employees.filter(EmployeeCode__icontains=EmpCode)
            leave_application = leave_application.filter(Emp_code__icontains=EmpCode)

        if selected_leave_ids:
            leave_application = leave_application.filter(Leave_Type_Master_id__in=selected_leave_ids)
        ...
        elif isinstance(selected_year, (list, tuple)):
            # If only one year selected → unwrap it
        ...
        if selected_year:
            if isinstance(selected_year, list):
                leave_application = leave_application.filter(
                    Start_Date__year__in=selected_year
                )
            else:
                leave_application = leave_application.filter(
        ...
        if selected_months in (None, '', 'All'):
            selected_months = None

        elif isinstance(selected_months, (list, tuple)):
            # Convert month names to numbers
            selected_months = [
                month_name_to_number.get(m) for m in selected_months
                if month_name_to_number.get(m)
            ]

        elif isinstance(selected_months, str):
            # Convert single month name to number
            selected_months = month_name_to_number.get(selected_months)
        ...
        emp_data = list(employees.values(
            ...
            'full_name',
            'EmployeeCode',
            'work_departmentorg_id',
            'EmpID',
            'work_department',
            'work_designation',
            'work_division',
            'work_date_of_joining'
        ))

        # ---------------------------------
        # Leave data
        # ---------------------------------
        leave_data = list(leave_application.values(
            'Emp_code',
            'leave_type',
            'Start_Date',
            'End_Date',
        ...
        # ---------------------------------
        # Attach leave rows to employees
        # ---------------------------------
        final_rows = []

        for emp in emp_data:
            for leave in leave_data:
                if leave['Emp_code'] == emp['EmployeeCode']:
                    row = emp.copy()
                    row.update({
                        'Leave': leave['leave_type'],
                        'From Date': leave['Start_Date'],
                        'To Date': leave['End_Date'],
                        'Days': leave['Total_credit'],
                        'Reason': leave['Reason'],
                        # 'Status': leave['Status'],
                    })
        ...
        df.sort_values(by=['full_name', 'From Date'], inplace=True)
...
elif selected_report == "Locker Allotment Report":
    df_Locker_Allotment_data = fetch_Locker_Allotment_data("Locker Allotment Report",report_options, org_id, selected_Division, selected_department, selected_designation,selected_year, selected_months,)
...
if 'df_Locker_Allotment_data' in locals() and not df_Locker_Allotment_data.empty:
    df_Locker_Allotment_data['work_date_of_joining'] = df_Locker_Allotment_data['work_date_of_joining'].apply(format_date)
    df_Locker_Allotment_data = df_Locker_Allotment_data.rename(columns={
        'full_name': 'Name',
        'work_departmentorg':'Hotel Name',
        'EmployeeCode': 'Emp Code',
        'work_division': 'Division',
...
def create_pdf_report_v1(data_frame, report_title, organization_name):
    buffer = BytesIO()
    ...
    max_width = 500 - left_margin - right_margin
...
def create_pdf_report(data_frame, report_title, organization_name):
    ...
    col_widths = []
    for col in headers:
        weight = max(len(col), 8)
        col_widths.append(base_width * min(weight / 12, 2))
...
if selected_report == "Locker Allotment Report":
    st.header("Locker Allotment Report")
    if 'df_Locker_Allotment_data' in locals() and not df_Locker_Allotment_data.empty:
        df_display = df_Locker_Allotment_data.drop(columns=['EmpID', 'work_departmentorg_id'])
        df_display.reset_index(drop=True, inplace=True)
        df_display.index += 1
        ...
        excel_data = convert_df_to_excel(df_Locker_Allotment_data)
        pdf_data = create_pdf_report(df_Locker_Allotment_data, "Locker Allotment Report", selected_org)

        col1, col2 = st.sidebar.columns(2)
        with col1:
            st.download_button(label="📥Download Excel", data=excel_data, file_name='locker_allotment_report.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        with col2:
            st.download_button(label="📥Download PDF", data=pdf_data, file_name='locker_allotment_report.pdf', mime='application/pdf')
    else:
        st.write("No new joinees data available.")     
...
if selected_report == "Upcoming Birthday's report":
    ...
    if 'df_Birthday_data' in locals() and not df_Birthday_data.empty:
        ...
        st.write("Upcoming birthday's data is not available.")
...
# Trial ------------
if selected_report == "Leave Consumed Report":
    df_leave_taken_data = fetch_leave_consumed_report(
        "Leave Consumed Report", report_options, org_id, selected_Division, selected_department, selected_designation, selected_year,  selected_months
    )
    st.header("Leave Consumed Report")

    if not df_leave_taken_data.empty:
        # Format the date and rename columns for display
        df_leave_taken_data['work_date_of_joining'] = df_leave_taken_data['work_date_of_joining'].apply(format_date)
        df_leave_taken_data = df_leave_taken_data.rename(columns={
            'full_name': 'Name',
            'EmployeeCode': 'Emp Code',
            'work_division': 'Division',
            'work_department': 'Department',
        ...
        df_leave_taken_data.reset_index(drop=True, inplace=True)
        df_leave_taken_data.index += 1

        # Add hyperlinks for the "View" column
        PyHost = MasterAttribute.PyHost  
        # df_leave_taken_data['View'] = df_leave_taken_data.apply(
        #     lambda row: f"{PyHost}HumanResources/PersonalDetails/?EmpID={encrypt_id(row['EmpID'])}&OID={row['work_departmentorg_id']}"
        #     if row['Emp Code'] else None,
        #     axis=1
        # )
        df_leave_taken_data = df_leave_taken_data.drop(columns=['EmpID', 'work_departmentorg_id'])
        ...
        excel_data = convert_df_to_excel(df_leave_taken_data)
        pdf_data = create_pdf_report(df_leave_taken_data, "Leave Consumed Report", selected_org)
        ...
        with col1:
            st.download_button(
                label="📥Download Excel",
                data=excel_data,
                file_name='leave_taken_report.xlsx',
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            )
        with col2:
            st.download_button(
                label="📥Download PDF",
                data=pdf_data,
                file_name='leave_taken_report.pdf',
                mime='application/pdf'
            )
    else:
        st.write("No leave taken data available.")
...
def fetch_masterreport_data(report_type, org_id, selected_Division, selected_department, selected_designation, selected_year, selected_months, selected_level, report_options):
    if report_type not in report_options:
        # raise ValueError(f"Invalid report type: '{report_type}'. Please select a valid report from: {report_options}")
        st.header(f"Please select a report from {report_options}")
    ...
    if report_type == "Master Report":
        ...
        if org_id is None:
                ...
                work_OfficialEmailAddress=Subquery(EmployeeWorkDetails.objects.filter(
                    EmpID=OuterRef('EmpID'),
                    IsDelete=False,
                ).values('OfficialEmailAddress')[:1]),
                
                work_OfficialMobileNo=Subquery(EmployeeWorkDetails.objects.filter(
                    EmpID=OuterRef('EmpID'),
                    IsDelete=False,
                ).values('OfficialMobileNo')[:1]),
                ...
                full_name=Concat('FirstName', Value(' '), 'LastName'),
                Personal_Mobile_Number=F('MobileNumber'),
                Personal_Email_Address=F('EmailAddress'),
                ...
                emp_emergency_contact_1=Subquery(EmployeeEmergencyInformationDetails.objects.filter(EmpID=OuterRef('EmpID')).values('EmergencyContactNumber_1')[:1]),
                emp_blood_group=Subquery(EmployeeEmergencyInformationDetails.objects.filter(EmpID=OuterRef('EmpID')).values('BloodGroup')[:1]),
                emp_Permanent_State=Subquery(EmployeeAddressInformationDetails.objects.filter(EmpID=OuterRef('EmpID')).values('Permanent_State')[:1]),
                emp_Permanent_City=Subquery(EmployeeAddressInformationDetails.objects.filter(EmpID=OuterRef('EmpID')).values('Permanent_City')[:1]),
                ctc=Coalesce(
                    Subquery(
        ...
        df = pd.DataFrame(list(employees.values(
            ...
            'work_department', "work_OfficialEmailAddress", 
            "work_OfficialMobileNo", 
            "Personal_Mobile_Number",
            "Personal_Email_Address",
            "emp_emergency_contact_1",
            "emergency_contact_2",
            "emp_blood_group",
            "emp_Permanent_State",
            "emp_Permanent_City",
            'work_designation', 'ReportingtoDesignation',
        ...
        if org_id ==  None:
            desired_order = [
                'work_departmentorg','EmployeeCode', 'full_name', 'work_division', 'work_department', "work_OfficialEmailAddress", 
                "work_OfficialMobileNo", 
                "Personal_Mobile_Number",
                "Personal_Email_Address",
                # "emergency_prefix",
                ...
                # "Permanent_State",
                # "Permanent_City",
                'work_designation', 'Level','ReportingtoDesignation', 
            ...
            desired_order = [
                'work_departmentorg','EmployeeCode', 'full_name', 'work_division', 'work_department', 
                "work_OfficialEmailAddress", 
                "work_OfficialMobileNo", 
                "Personal_Mobile_Number",
                "Personal_Email_Address",
                "emp_emergency_contact_1",
...
def fetch_identity_info(emp_ids, org_id):
    qs = EmployeeIdentityInformationDetails.objects.filter(
        EmpID__in=emp_ids,
        OrganizationID=org_id,
        IsDelete=False
    ).values(
...
def fetch_Bank_info(emp_ids, org_id):
    qs = EmployeeBankInformationDetails.objects.filter(
        EmpID__in=emp_ids,
        OrganizationID=org_id,
        IsDelete=False
    ).values(
...
if selected_report == "Master Report":
    ...
    def format_date(date_value):
        try:
            ...
            # return datetime.strptime(str(date_value)[:10], "%Y-%m-%d").strftime("%d %b %Y")
            return datetime.datetime.strptime(str(date_value)[:10], "%Y-%m-%d").strftime("%d %b %Y")
    ...
    master_data = master_data.rename(columns={
        ...
        'work_department': 'Department',
        'work_division': 'Division',
        "work_OfficialEmailAddress": "Official Email Address", 
        "work_OfficialMobileNo":"Official Mobile No",
        ...
        "emp_emergency_contact_1":"emergency contact 1",
        "emergency_contact_2":"emergency contact 2",
        "emp_blood_group":"Blood Group",
        "emp_Permanent_State":"Permanent State",
        "emp_Permanent_City":"Permanent City",
        'work_departmentorg':'Hotel Name',
        'work_designation': 'Designation',
    ...
    if emp_status == "Select Employee Status":
        emp_status = "All"
    ...
    #  ------------------ New Sections Identity_Information_Details -------------------
    Identity_Information_Details = st.sidebar.checkbox("Identity Information Details")
    if Identity_Information_Details:
        show_pan = True
        show_aadhaar = True
    else:
        show_pan = False
    ...
    if show_pan or show_aadhaar:
        emp_ids = master_data['EmpID'].unique().tolist()

        identity_df = fetch_identity_info(emp_ids, org_id)
    ...
    # PAN
    if show_pan:
        master_data.rename(columns={'PANNo': 'PAN Number'}, inplace=True)
    else:
        master_data = master_data.drop(columns=['PANNo'], errors='ignore')

    # Aadhaar
    if show_aadhaar:
        master_data.rename(columns={'AadhaarNumber': 'Aadhaar Number'}, inplace=True)
    else:
        master_data = master_data.drop(columns=['AadhaarNumber'], errors='ignore')

    
    # ----------------- / New Sections End Identity_Information_Details -------------------
    
  
    #  ------------------ Bank_Information_Details -------------------
    Bank_Information_Details = st.sidebar.checkbox("Bank Information Details")
    # if Bank_Information_Details:
    #     show_pan = True
    #     show_aadhaar = True
    # else:
    #     show_pan = False
    ...
    if Bank_Information_Details:
        ...
        bank_df = fetch_Bank_info(emp_ids, org_id)
...
if selected_report == "Employees History":
    ...
    dfs = [
        employeeper_data, letter_data, probation_confirmationconfirmationletter,
    ...
    sheet_names = [
        "Employee History", "Letters", "Probation Confirmation",
