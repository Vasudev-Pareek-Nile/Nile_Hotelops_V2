...
# # Leave Taken Report
def fetch_leave_taken_report(report_type, report_options, org_id, selected_Division, selected_department, selected_designation, selected_year, selected_months):
    if report_type not in report_options:
        raise ValueError(
            f"Invalid report type: '{report_type}'. Please select a valid report from: {report_options}"
        )

    if report_type == "Leave Taken Report":
...
# Trial ------------
if selected_report == "Leave Taken Report":
    df_leave_taken_data = fetch_leave_taken_report(
        "Leave Taken Report", report_options, org_id, selected_Division, selected_department, selected_designation, selected_year,  selected_months
    )
    st.header("Leave Taken Report")
    ...
    if not df_leave_taken_data.empty:
        ...
        pdf_data = create_pdf_report(df_leave_taken_data, "Leave Taken Report", selected_org)
