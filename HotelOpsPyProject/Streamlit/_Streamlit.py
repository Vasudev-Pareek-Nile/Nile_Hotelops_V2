...
from Manning_Guide.models import OnRollDesignationMaster,OnRollDepartmentMaster,LavelAdd  
...
# query_params = st.query_params  # Retrieve query parameters
# user_id = query_params.get("token", [None])[0]  # Clear the copy if needed
#    # Correct way to get query params



# user_id = query_params.get("token", [None])[0]
...
#     st.stop()  # Stop the app and prevent any further content from loading
# else:
#     # Proceed with the rest of your app logic if the user_id is valid
#     st.write(f"Welcome! Token received: {user_id}")


# if user_id:
#     API_URL = f"http://127.0.0.1:8000/api/user-session/{user_id}/"
#     auth_token = "YOUR_AUTH_TOKEN_HERE"  
#     headers = {
#         "Authorization": f"Bearer {auth_token}"  
#     }
...
#         # Handle the organization selection logic
#         if OID == '3':
#             memOrg = OrganizationMaster.objects.filter(IsDelete=False, Activation_status='1').values('OrganizationID', 'OrganizationName')
#             organization_list = [org['OrganizationName'] for org in memOrg]
#             organization_ids = [str(org['OrganizationID']) for org in memOrg]
...
#         # Sidebar select box for organization selection
#         selected_org = st.sidebar.selectbox(
#             "Choose an organization", 
#             organization_list, 
#             index=organization_ids.index(str(st.session_state.organization_id)) if str(st.session_state.organization_id) in organization_ids else 0
...
#         # Fetch organization data based on the selected organization
#         if org_id is None:
#             all_org_data = OrganizationMaster.objects.filter(IsDelete=False, Activation_status='1').values('OrganizationID', 'OrganizationName')
#         else:
#             specific_org_data = OrganizationMaster.objects.filter(OrganizationID=org_id).values('OrganizationID', 'OrganizationName')
...
#     else:
#         st.write(f"Error: Could not fetch user session data. Status Code: {response.status_code}")
...
selected_module = st.sidebar.selectbox("Choose a Module", module_names, index=module_names.index('Human Resources') if 'Human Resources' in module_names else 0)


if selected_module == 'All':
    report_options = ReportModuleMaster.objects.filter(IsDelete=False).values_list('Report_title', flat=True).distinct()
else:
    report_options = ReportModuleMaster.objects.filter(Module_Name=selected_module, IsDelete=False).values_list('Report_title', flat=True)


report_options = list(report_options)


selected_report = st.sidebar.selectbox("Choose a report type", report_options, index=report_options.index('Master Report') if 'Master Report' in report_options else 0)
...
def fetch_filtered_data(report_type, report_options, org_id, selected_department, selected_designation, selected_year, selected_months, selected_resignation_reason=None):
    ...
    if report_type == "Employees on Resignation":
        ...
        if not data.empty:
            ...
            lambda row: f"{Host}HumanResources/PersonalDetails/?EmpID={encrypt_id(row['EmpID'])}&OID={row['work_departmentorg_id']}" if row['EmpID'] else None, axis=1
            )
        else:
...
def fetch_new_joinees_data(report_type, org_id, selected_department, selected_designation, selected_year, selected_months, report_options):
    ...
    if report_type == "New Joinees Report":
        ...
        if not data.empty:
            data['View'] = data.apply(
                lambda row: f"{Host}HumanResources/EditEmployee/?EmpID={encrypt_id(row['EmpID'])}&OID={row['work_departmentorg_id']}", axis=1
            )
        else:
            print("The DataFrame is empty.")
...
def fetch_new_Emergency_data(report_type,report_options, org_id,selected_department, selected_designation,selected_year, selected_months,):
    ...
    if report_type == "Emergency Contact Report":
        ...
        columns_order = ['work_departmentorg', 'EmployeeCode', 'full_name', 
                        'work_department', 'work_designation', 'work_departmentorg_id','EmpID',
...
def fetch_blood_data(report_type, report_options,org_id,selected_department, selected_designation,selected_year, selected_months,):
    ...
    if report_type == "Blood Group Report":
       ...
       # Define the list of columns you want
        ...
        if not employee_data.empty:
            employee_data['View'] = employee_data.apply(
            lambda row: f"{Host}HumanResources/EmergencyInfoPage/?EmpID={encrypt_id(row['EmpID'])}&OID={row['work_departmentorg_id']}", axis=1
            )
        else:
            print("The DataFrame is empty.")  
...
def fetch_Birthdays_data(report_type, report_options, org_id, selected_year, selected_months, selected_department, selected_designation):
    ...
    if report_type == "Upcoming Birthday's report":
        ...
        if not employee_data.empty:
            employee_data['View'] = employee_data.apply(
            lambda row: f"{Host}HumanResources/PersonalDetails/?EmpID={encrypt_id(row['EmpID'])}&OID={row['work_departmentorg_id']}", axis=1
             )
        else:
            print("The DataFrame is empty.")   
        return employee_data
...
def fetch_leave_balance_report(report_type, report_options, org_id, selected_department, selected_designation):
    ...
    if report_type == "Leave balance report":
        ...
        emp_data_df['View'] = emp_data_df.apply(
            lambda row: f"{Host}HumanResources/PersonalDetails/?EmpID={encrypt_id(row['EmpID'])}&OID={row['work_departmentorg_id']}" 
...
if selected_report == "Employees on Resignation":
    ...
    df_resignation = fetch_filtered_data(
        "Employees on Resignation", report_options, org_id, selected_department, 
    ...
    df_termination = fetch_filtered_data(
        "Employees on Termination", report_options, org_id, selected_department, 
        selected_designation, selected_year, selected_months
    )


elif selected_report == "New Joinees Report":
    df_new_joinees = fetch_new_joinees_data( "New Joinees Report", org_id, selected_department,
    ...
    df_new_Emergency_data = fetch_new_Emergency_data(
        "Emergency Contact Report", report_options, org_id, selected_department, 
        selected_designation, selected_year, selected_months
    )

elif selected_report == "Blood Group Report":
    df_blood_data = fetch_blood_data("Blood Group Report",report_options, org_id,selected_department, selected_designation,selected_year, selected_months,)

elif selected_report == "Absconding Employees Report":
    df_Absconding_data = fetch_absconding_data("Absconding Employees Report",report_options, org_id,selected_department, selected_designation,selected_year, selected_months,)



elif selected_report == "Terminated Employees":
    df_TerminateEmployees_data = fetch_TerminateEmployees_data("Terminated Employees",report_options, org_id,selected_department, selected_designation,selected_year, selected_months,)


elif selected_report == "Upcoming Birthday's report":
    df_Birthday_data = fetch_Birthdays_data("Upcoming Birthday's report",report_options, org_id,selected_year, selected_months,selected_department, selected_designation)

elif selected_report == "Actual Salary OnRoll Report":
    df_headsalary_data = fetch_Acutalcounthr("Actual Salary OnRoll Report",report_options, org_id,selected_year, selected_months,selected_department, selected_designation)


elif selected_report == "Actual Head Count OnRoll Report":
    df_headHeadCount_data = fetch_AcutalHeadCounthr("Actual Head Count OnRoll Report",report_options, org_id,selected_department, selected_designation)



elif selected_report == "Actual Contract Report":
    df_ActualContract_data = fetch_ActualContracthr("Actual Contract Report",report_options, org_id,selected_department, selected_designation)
    ...
    df_Notice_data = fetch_Notice_data("Employees Serving Notice Period",report_options, org_id,selected_department, selected_designation,selected_year, selected_months)       

elif selected_report == "Exit Interview Report":
    df_exit_interview_data = fetch_exit_interview_data("Exit Interview Report",report_options, org_id,selected_department, selected_designation)       
...
if selected_report == "Leave balance report":
    ...
    if not df_leave_balance_data.empty:
        ...
        Host = MasterAttribute.Host  # Replace with actual host if needed
...
employees=[]
def fetch_masterreport_data(report_type, org_id, selected_department, selected_designation, selected_year,    selected_months, selected_level, report_options):
    if report_type not in report_options:
        raise ValueError(f"Invalid report type: '{report_type}'. Please select a valid report from: {report_options}")
    ...
    if report_type == "Master Report":
        ...
        if org_id is None:
                     ...
                     employees = EmployeePersonalDetails.objects.annotate(
                        ...
                        empappoinment_match = Subquery(
                            ...
                            .annotate(date_only=TruncDate('date_of_appointment')) 
                            .values('date_only')[:1]  
                        ),
                    ...
                    #    resignation module 
                        ...
                        resigntionsdate = Subquery(
                            ...
                            .annotate(date_only=TruncDate('Date_Of_res')) 
                            .values('date_only')[:1] ),
        ...
        df = pd.DataFrame(list(employees.values(
            ...
            'work_departmentorg','EmployeeCode','full_name', 
            'work_department', 'work_designation', 'ReportingtoDepartment', 
        ...
        if org_id ==  None:
            ...
            desired_order = [
                ...
                'work_departmentorg','EmployeeCode', 'full_name', 'work_department', 'work_designation', 'Level', 'ReportingtoDepartment', 
...
if selected_report == "Master Report":
    ...
    # def format_date(date_value):
    #     try:
    #         # Attempt to parse the date and format it as '21 Jan 2024'
...
def fetch_EmployeesHistory_data(report_type, org_id, report_options, filters):
    ...
    if report_type == "Employees History":
        ...
        employee_df = pd.DataFrame(list(employees.values(
            'full_name', 'EmployeeCode', 'work_department', 'work_designation', 'ReportingtoDepartment', 
            'work_date_of_joining', 'work_status'
        )))
