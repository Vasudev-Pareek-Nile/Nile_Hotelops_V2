...
def GuestVoice_Dashboard(request):
    ...
    if organization.ReviewSoftware == "reviewPro":
       ...
       Medallia = 'Show'
    elif organization.ReviewSoftware == "MedalliaWyndham":
       MedalliaWyndham = 'Show'
    else:
    ...
    context = {
        ...
        'ReviewPro':ReviewPro,
        'Medallia':Medallia,
        'MedalliaWyndham':MedalliaWyndham,
...
def Medallia_Data_List(request):
    ...
    selected_software = request.GET.get('SW')

    if not selected_software:
        selected_software = 'NoSoftware'
...
def clean_row_data_madallia(row_data):
    decimal_fields = [
        'DeliveryHkServices', 'CustomerService', 'ConditionOfHotel',
    ...
    if request.method == "POST" and request.FILES.get("excel_file_upload"):
        ...
        for _, row in df.iterrows():
            ...
            if not pd.notnull(entry_date):
                # print("Skipped row because PublishedDate is null:", row_data)
                continue  # skip this iteration, don’t insert into DB
...
def bulk_upload_medallia(request):
    if request.method == "POST" and request.FILES.get("excel_file_upload"):
        ...
        # OrganizationID = request.session["OrganizationID"]
        OrganizationID = request.GET.get('OID') 
        SoftwareW = request.GET.get('SW')
        
        response = None

        if SoftwareW == 'MedalliaWyndham':
            response = bulk_upload_medallia_Wyndham(request,file,UserID,OrganizationID, SoftwareW)
        else:
            response = bulk_upload_medallia_V1(request,file,UserID,OrganizationID)

    print("Response is here:",response.url)
    return response
    # return redirect('Medallia_Data_List')



def bulk_upload_medallia_Wyndham(request,file,UserID,OrganizationID, SoftwareW):

        # print("#####################----- Wyndham Meddallia ------ #########################")
        DateTimeObj = datetime.now()
        DateAndTime =  DateTimeObj.date()
        ...
        Column_Rename = {
            ...
            'Check-in':'CheckInProcess',
            'Property Appearance':'ConditionOfHotel',    
            'Breakfast':'BreakfastExperience',
            # 'Overall WOH Program Experience':'WohProgramExperience',   
            'Food & Beverage':'OverallFnbExperience',
            ...
            'All survey comments concatenated':'Comments'
        }
        ...
        for _, row in df.iterrows():
            ...
            # print("Before cleaning NPS:", row_data.get("NPS"))
            row_data = clean_row_data_madallia(row_data)
            # print("After cleaning NPS:", row_data.get("NPS"))
        ...
        #  redirect('Medallia_Data_List')


def bulk_upload_medallia_V1(request,file,UserID,OrganizationID):
