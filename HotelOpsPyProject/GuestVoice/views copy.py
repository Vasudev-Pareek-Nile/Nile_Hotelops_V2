...
def bulk_upload_medallia(request):
    if request.method == "POST" and request.FILES.get("excel_file_upload"):
        ...
        df = df.dropna(how='all')
# 
        # df = pd.read_excel(file, keep_default_na=False, na_values=[''], dtype=str)
        # df = pd.read_excel(file)
        # df.columns = df.columns.str.strip()
        # df = df.dropna(how='all')
        ...
        # Universal cleanup for all columns
        # for col in df.columns:
        #     df[col] = df[col].apply(
        #         lambda x: None if pd.isna(x) or str(x).strip() == ""  
        #         else ("N/A" if str(x).strip().upper() == "N/A"        
        #             else (try_convert_number(x)))                  
        #     )
        ...
        Column_Rename = {
            ...
            'Staff Resposiveness to Guest Needs':'StaffResponsiveness',
            'Satisfaction with LP Member Benefits':'LpMemberSatisfaction',
            'WOH App Experience':'WohAppExperience',
            'Overall Delivery of HK Services':'DeliveryHkServices',
            'Comments':'Comments'
        }
