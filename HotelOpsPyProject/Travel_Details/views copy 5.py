...
class TravelRequestViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        # Collect current and incoming entry IDs
        current_entries = {entry.id: entry for entry in instance.entries.all()}
        incoming_ids = set()

        for entry_data in entries_data:
            entry_id = entry_data.get('id')

            if entry_id and entry_id in current_entries:
        ...
        # Delete entries not included in request
        ...
        return Response({"Detail": "The entry is updated successfully."}, status=status.HTTP_200_OK)
