...
def Travel_Details_List(request):
    ...
    defaultmonth = datetime.now().strftime("%B")
    ...
    month = request.GET.get('month', defaultmonth).strip()
    travel_mode = request.GET.get('travel_mode', '').strip()

    print("Selected Month:", month)
    print("Default Month:", defaultmonth)
    ...
    # if not month:  
    #     # Default to current month name, e.g. "September"
    #     month = datetime.now().strftime("%B")

    if month:
        try:
            # Get only the first word (month name)
            month_name = month.split("-")[0].strip()
    ...
    selected_month_number = datetime.strptime(month.split("-")[0].strip(), "%B").month
    months = [f"{m} - {datetime.now().year}" for m in [
...
from .permissions import HasValidUserAndOrgID

class TravelRequestViewSet(viewsets.ModelViewSet):
    permission_classes = [HasValidUserAndOrgID]
    ...
    ordering_fields = ['created_datetime']  # Allow ordering by this field
    ordering = ['-created_datetime']  # Default ordering (latest first)
    ...
    def update(self, request, *args, **kwargs):
        ...
        # Soft delete the existing TravelRequest
        old_instance.is_delete = True
        old_instance.deleted_by = request.user.id if request.user.is_authenticated else None
        old_instance.deleted_datetime = timezone.now()
        old_instance.save()

        # Prepare data for new TravelRequest creation
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        validated_data = serializer.validated_data
        entries_data = validated_data.pop('entries', [])

        # Create new TravelRequest
        new_request = TravelRequest.objects.create(**validated_data)

        # Create new entries
        for entry_data in entries_data:
            TravelEntry.objects.create(travel_request=new_request, **entry_data)

        return Response({"Detail": "Record Deleted Successfully."}, status=status.HTTP_202_ACCEPTED)
