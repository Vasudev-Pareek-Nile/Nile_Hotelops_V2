    ...
    if InterviewID is not None:
        ...
        if IntentObj is not None:
            ...
            for salary in SalaryTitles:
                salary.Permonth = 0
                salary.Perannum = 0
                SC = SalaryDetails.objects.filter(IsDelete=False,LETTEROFINTENTEmployeeDetail=IntentObj, Salary_title_id = salary.id,OrganizationID=OID)
                if SC.exists():
    ...
    if request.method  == "POST":
        ...
        if DataFromIntentObj == "IntentObj"  :
            if Reissue == "Yes":
                ...
                IntentObj.save()
                SC = SalaryDetails.objects.filter(IsDelete=False,LETTEROFINTENTEmployeeDetail=IntentObj,OrganizationID=OID)
                for s in SC:
                        s.IsDelete = True
                        s.ModifyBy  = UserID
                        s.save()
