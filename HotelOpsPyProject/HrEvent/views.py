...
def index(request):
    ...
    return render(request,'hrevent/index.html',context)
...
def all_events(request):
    ...
    all_events = HrEventCalendar.objects.filter(IsDelete=False, OrganizationID=I)
    event_list = []
...
def add_event(request):
    ...
    if id=='0':
        event = HrEventCalendar(name=str(title), start=start,end = end,OrganizationID=orgid,CreatedBy=UserID)
        event.save()
    else:
        event = HrEventCalendar.objects.get(id=id)
        event.start = start
        event.end = end
