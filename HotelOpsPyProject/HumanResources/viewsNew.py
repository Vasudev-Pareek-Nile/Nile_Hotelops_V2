...
def HumanResourcesDashboard(request):
    ...
    context = {
        ...
        'OrganizationID': OrganizationID,
        'selectedOrganizationID': selectedOrganizationID,
        'memOrg': memOrg  # Optional: only if needed in template
    }
...
def NewEmployeeDataForm(request):
    ...
    #     if Mobj is None:
    #         # - redirect, show an error, or return a default value
    #         return HttpResponse("No employee data found.", status=404)
    ...
    if request.method == "POST":
                ...
                if HrEmobj  is not None:
                    ...
                    HrEmobj  = EmployeePersonalDetails.objects.create(EmployeeCode = EmployeeCode,
                     Prefix  = Prefix,FirstName = FirstName,MiddleName = MiddleName,LastName  = LastName,Gender = Gender,MaritalStatus   = MaritalStatus ,DateofBirth  = DateofBirth ,age  = age,MobileNumber  = MobileNumber,EmailAddress  = EmailAddress,CovidVaccination = CovidVaccination,DetailsofIllness = IllnessDetails,OrganizationID = OrganizationID,CreatedBy = UserID,
                     InterviewAssessmentID=IND, Source=D)
                    if Pre_EmpID:
    ...
    'CovidVaccination': HrEmobj.CovidVaccination if HrEmobj and HrEmobj.CovidVaccination else '',
    ...
    context = {'Emobj':EmobjhrDict,'ProfileImagePreview':ProfileImagePreview,'ActiveLinkDict':ActiveLinkDict,'CurrentUrl':CurrentUrl,'D':D,'Nexturl':Nexturl,'EmpID':EmpID,'IND':IND,'hotelapitoken':hotelapitoken,'OrganizationID':OrganizationID}
...
def NewEmployeePersonalData(request):
    ...
    context = {'Emobj':HrEmobj,'ProfileImagePreview':ProfileImagePreview,'ActiveLinkDict':ActiveLinkDict,'CurrentUrl':CurrentUrl,'D':D,'Nexturl':Nexturl,'EmpID':EmpID,'IND':IND,'hotelapitoken':hotelapitoken,'OrganizationID':OrganizationID}
    return render(request, 'HR/NewEmployeeAdd/NewEmployeePersonalData.html', context)
...
def NewEmployeeWorkData(request):
    ...
    if request.method == "POST":
                ...
                if Workobj  is not None and Datafrom  == 'DatafromHR':
                    ...
                    Workobj = EmployeeWorkDetails.objects.create(EmpID = EmpID,
                         Designation = Designation,Department =Department,Level=Level,ReportingtoDesignation = ReportingtoDesignation,ReportingtoDepartment =ReportingtoDepartment,ReportingtoLevel  = ReportingtoLevel,OfficialEmailAddress = OfficialEmailAddress,OfficialMobileNo = OfficialMobileNo,DateofJoining = DateofJoining,CompanyAccommodation = CompanyAccommodation,Locker = Locker,LockerType = LockerType,LockerNumber = LockerNumber,EmploymentType  =EmploymentType,ContractEndDate = ContractEndDate,ContractStartDate =  ContractStartDate, OrganizationID = OrganizationID,CreatedBy = UserID,DottedLine=DottedLine,VipCheckbox=VipCheckbox,WeekOffDay=WeekOffDay
                    )
...
def NewEmployeeFamilyinfo(request):
                ...
                if FamilyObj is not None:
                    ...
                    for id, name, age, relation in zip(child_update_ids, child_names, child_ages, child_relations):
                     ...
                     FamilyObj = EmployeeFamilyDetails.objects.create(
                                ...
                                ,FatherAge  = FatherAge
                                ,FatherContactNo = FatherContactNo,LandlineNo=LandlineNo
                                ,OrganizationID = OrganizationID,CreatedBy = UserID                       
                     )
    ...
    if EmpID:
         ...
         if Employeeobj.MaritalStatus ==  'Unmarried':
                SpouseChild = False 
    context = {'SpouseChild':SpouseChild,'ActiveLinkDict':ActiveLinkDict,'CurrentUrl':CurrentUrl,'Nexturl':Nexturl,'EmpID':EmpID,'PreviousUrl':PreviousUrl,'IND':IND,'D':D,
...
def NewEmployeeSalaryDetailsData(request):
    ...
    for salary in SalaryTitles:
        ...
        if SC.exists():
                ...
                SC = SalaryDetails.objects.filter(IsDelete=False,LETTEROFINTENTEmployeeDetail__InterviewID=IND, Salary_title_id = salary.id,OrganizationID=OrganizationID)
                if SC.exists():
                    salary.Permonth = SC[0].Permonth
                    salary.Perannum = SC[0].Perannum 
    ...
    if request.method == "POST":
                ...
                for i in range(int(Total_Title)+1):
                    ...
                    salaryObj = Salary_Detail_Master.objects.create(
                        ...
                        Permonth=Permonth,Perannum=Perannum,OrganizationID=OrganizationID,CreatedBy  = UserID
                    )
    ...
    context = {'ActiveLinkDict':ActiveLinkDict,'CurrentUrl':CurrentUrl,'Nexturl':Nexturl,'EmpID':EmpID,'PreviousUrl':PreviousUrl,'IND':IND,'D':D,'SalaryTitles' :SalaryTitles            
    }
...
from .Reports import NewJoineeRerports,EmergencyInformation
...
from .Reports import NewJoineeRerports,EmergencyInformation
from Manning_Guide.models import OnRollDepartmentMaster,OnRollDivisionMaster
def EmployeeList(request):
    ...
    selected_emp_status = request.GET.get("EmpStatus", "all")
    selected_department = request.GET.get("Department")
    selected_division = request.GET.get("DivisionName")
    employee_code = request.GET.get("employee_code")
    if UserType == 'CEO' and request.GET.get('OrganizationID') is None:
            selected_org_id = 401
 
    print("Employee Status:", selected_emp_status)
    ...
    if selected_division and selected_division != "AllDivision":
        ...
        departments_in_division = Departments
    ...
    context = {
        ...
        'selected_department': selected_department,  # Pass the selected department
        'selected_division': selected_division,
        'employee_code': employee_code,
        'departments_in_division':departments_in_division
...
def DepartmentJson():
     ...
     for merged in MergedDesignations:
        if hasattr(merged, 'OnRollDepartmentMaster') and merged.OnRollDepartmentMaster:
        ...
        elif hasattr(merged, 'CorporateDepartmentMaster') and merged.CorporateDepartmentMaster:
            merged_designations_data.append({
                'designations': merged.designations,
...
def  EmployeeWorkDetailsPage(request):
     ...
     if request.method  ==  "POST":
                ...
                if Workobj  is not None:
                    ...
                    if DesignationList and DepartmentList and IsDeleteList and IdList:
                        for id, designation, department, is_delete in zip(IdList, DesignationList, DepartmentList, IsDeleteList):
     ...
     context = {'OrganizationID':OrganizationID,'MergedReportingtoDesignation':MergedReportingtoDesignation,'Workobj':Workobj,'Emobj':Emobj,'EmpID':EmpID,'Success':Success,'MergedDottedLineDesignations':MergedDottedLineDesignations , 'MergedDesignations':MergedDesignations
               , 'merged_designations_json':merged_designations_json,'subrecordslist_json':subrecordslist_json,
                }
...
def  SalaryDetailsPage(request):
     ...
     context = {'Emobj':Emobj,'EmpID':EmpID,'Success':Success, 'SalaryTitles':SalaryTitles,'OrganizationID':OrganizationID}
...
def UpdateEmployeeSalaryGridByPromotion(EmpID, Promotionobj, OrganizationID, UserID):
    try:
        ...
        promotion_salaries = PromotionSalaryDetails.objects.filter(PromotionLetterEmployeeDetail=Promotionobj,OrganizationID=OrganizationID,IsDelete=False)

        for promo_salary in promotion_salaries:
          
            per_annum_salary = promo_salary.RevisedSal * 12  

            
            newsal = Salary_Detail_Master.objects.create(
                EmpID=EmpID,
                Salary_title_id = promo_salary.Salary_title.id,
                Permonth=promo_salary.RevisedSal,
...
def EmployeeLetters(request):
     ...
     AdvanceSalaryShow = 'Show'
     AdvanceSalary  = AdvanceSalaryForm.objects.filter(OrganizationID=OrganizationID,is_delete=False,emp_code= EC)
     if  AdvanceSalary.count() >0:
          AdvanceSalaryShow = 'Hide'
        
     print("AdvanceSalaryForm is here:", AdvanceSalary)
        
      
     context = {'Emobj':Emobj,'EmpID':EmpID,'EC':EC,'AppointemetLetters':AppointemetLetters,'Succcess':Succcess,'ExperienceLetters':ExperienceLetters,'last_Increamen_id':last_Increamen_id,'PromotionLetters':PromotionLetters,'last_Promotion_id':last_Promotion_id,'IncreamentLetters':IncreamentLetters,'RevealingLetters':RevealingLetters,'Appointemet':Appointemet,'OrganizationID':OrganizationID,
     'Experienceshow':Experienceshow,'Revealingshow':Revealingshow, 'AdvanceSalary':AdvanceSalary
     }
...
def Absconding(request):
     ...
     context = {'Emobj':Emobj,'EmpID':EmpID,'EC':EC,'Abscondings':Abscondings,'Notices':Notices,'OrganizationID':OrganizationID,'Noticeshow':Noticeshow,'Abscondingshow':Abscondingshow}
     return render(request, 'HR/EmployeeDashboard/Absconding.html', context)
...
def KraReport(request):
     ...
     context = {
            ...
            'Emobj':Emobj,'EmpID':EmpID,'EC':EC,
        
            'rowslist': rowslist,'OrganizationID':OrganizationID
