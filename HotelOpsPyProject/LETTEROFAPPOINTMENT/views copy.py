...
# def build_and_save_loa_html(emp_obj, OID, CreatedBy):
#     get_data =LETTEROFAPPOINTMENT.objects.get(id=id)
#     html = get_data.data

#     html = html.replace("@@date_of_appointment@@", emp_obj.date_of_appointment.strftime("%d-%B-%Y"))
#     html = html.replace("@@emp_code@@", emp_obj.emp_code)
#     html = html.replace("@@prefix@@", emp_obj.prefix)
#     html = html.replace("@@firstname@@", emp_obj.first_name)
#     html = html.replace("@@lastname@@", emp_obj.last_name)
#     html = html.replace("@@MobNo@@", emp_obj.mobile_number)
#     html = html.replace("@@Email@@", emp_obj.email)
#     html = html.replace("@@Designation@@", emp_obj.designation)
#     html = html.replace("@@Date_of_Joining@@", emp_obj.date_of_joining.strftime("%d-%B-%Y"))
#     html = html.replace("@@Grade@@", emp_obj.level)
#     html = html.replace("@@Basic_Salary@@", str(emp_obj.basic_salary))
#     html = html.replace("@@Department@@", emp_obj.department)
#     html = html.replace("@@Address@@", emp_obj.address)
#     html = html.replace("@@PageBreak@@", '<div style="page-break-after:always"></div>')


#     # ---------- Build Salary Table from LOA table ----------
#     SalaryTitles = SalaryTitle_Master.objects.filter(
#         IsDelete=False, OrganizationID=OID
#     ).order_by("TypeOrder", "TitleOrder")

#     loa_salary = Letter_Of_Appointment_Salary_Detail_Master.objects.filter(
#         EmpID=emp_obj.EmpID, OrganizationID=OID, IsDelete=False
#     ).select_related("Salary_title")

#     salary_dict = {s.Salary_title_id: s for s in loa_salary}

#     s = """
#     <table style="width:100%; border:1px solid #939191; border-collapse:collapse;">
#         <thead>
#             <tr style="background-color:#F6F1E4">
#                 <th>Components</th>
#                 <th>Per Month</th>
#                 <th>Per Annum</th>
#             </tr>
...
#     for st in SalaryTitles:
#         row = salary_dict.get(st.id)
#         per_month = row.Permonth if row else 0
#         per_annum = row.Perannum if row else 0
#         is_bold = st.IsBold
...
#                 <td>{st.Title}</td>
#                 <td align='right'>{per_month}</td>
#                 <td align='right'>{per_annum}</td>
#             </tr>
#             """
#         else:
#             s += f"""
...
#     html = html.replace("@@Salary_Details@@", s)

#     # Save Final HTML
#     emp_obj.FinalHTML = html
#     emp_obj.ModifyDateTime = timezone.now()
#     emp_obj.save()
...
def build_and_save_loa_html(emp_obj, OID, CreatedBy):
    ...
    template = LETTEROFAPPOINTMENT.objects.filter(user=1).first()
    html = template.data
    DOA = emp_obj.date_of_appointment
    print("Date of Appointment is here", DOA) 

    # 2. Replace tokens
    repl = {
        "@@date_of_appointment@@":fmt(emp_obj.date_of_appointment).strftime("%d-%B-%Y"),
        "@@emp_code@@": emp_obj.emp_code,
        "@@prefix@@": emp_obj.prefix,
        "@@firstname@@": emp_obj.first_name,
        "@@lastname@@": emp_obj.last_name,
        "@@MobNo@@": emp_obj.mobile_number,
        "@@Email@@": emp_obj.email,
        "@@Designation@@": emp_obj.designation,
        "@@Date_of_Joining@@": safe_date(emp_obj.date_of_joining).strftime("%d-%B-%Y"),
        "@@Grade@@": emp_obj.level,
        "@@Basic_Salary@@": fmt(emp_obj.basic_salary),
    ...
    # 3. Salary Table
    html = html.replace("@@Salary_Details@@", build_salary_table(emp_obj, OID))

    # 4. Save final static HTML
    emp_obj.FinalHTML = html
    emp_obj.ModifyDateTime = timezone.now()
    emp_obj.save()
...
def run_all_background(emp_obj, EmpID, OID, CreatedBy):
    """Runs salary dump + HTML build in ONE thread"""
    dump_salary_details_for_loa_once(EmpID, OID, CreatedBy)
    build_and_save_loa_html(emp_obj, OID, CreatedBy)



def run_background_async(emp_obj, EmpID, OID, CreatedBy):
    thread = threading.Thread(
        target=run_all_background,
        args=(emp_obj, EmpID, OID, CreatedBy),
        daemon=True
    )
    thread.start()
...
def entryEmp(request):
    ...
    if request.method == "POST":
            ...
            if DataFromAppointmentobj == "Appointmentobj" and LOA_ID:
                ...
                run_background_async(NewAppointmentobj, EmpID, OrganizationID, UserID)
...
def emplist(request):
    ...
    # AppointemetLetters = LOALETTEROFAPPOINTMENTEmployeeDetail.objects.filter(IsDelete=False,OrganizationID=I).order_by('-CreatedDateTime','-ModifyDateTime').values()
    ...
    print(LOALETTEROFAPPOINTMENTEmployeeDetail._meta.get_fields())


    print(list(AppointemetLetters.values('CreatedDateTime')[:5]))



    context  = { 'AppointemetLetters':AppointemetLetters,'I':I,'memOrg':memOrg}
...
def Generate_Appointment_Letter(request):
    ...
    id = request.GET["LOA_ID"]
    EmpID = request.GET["EmpID"]
    head = request.GET.get("head", "true") == "true"
