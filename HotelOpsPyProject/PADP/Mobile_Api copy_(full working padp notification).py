...
class PADP_Approve_Mobile_API(APIView):
    ...
    def get(self, request):
        ...
        # # NEW ACCESS CHECK
        # UserID = request.GET.get("UserID")
        UserID = (request.GET.get("UserID") or "").strip()
        # ALLOWED_USER_IDS = ['20201212180048']
        ALLOWED_USER_IDS = ['20201212180048','20251209112591']
        ...
        # print("select year is here::", selected_year)
        ...
        for rating in ratings:
            ...
            final_results.append({
                ...
                "SIO": salary_increment_option,
                # "Approval": rating.APADP.Approval_stage_Mobile_Api(),
                "LAS": rating.APADP.LastApporvalStatus,
                "CreatedOnRaw": rating.CreatedDateTime,
                # "MDT": rating.APADP.ModifyDateTime,
                "MD": rating.APADP.ModifyDateTime.strftime("%Y-%m-%d"),
                "MT": rating.APADP.ModifyDateTime.strftime("%I:%M %p"),
                "id": rating.APADP.id,
                "Type": "A",
                "URL": f"https://hotelops.in:8080/PADP/ApadpPdf/?ID={rating.APADP.id}&Page=PADPApprove"   
            })
        ...
        for entry in entry_records:
            ...
            final_results.append({
                ...
                # "ceo_as": entry.ceo_as,
                "CreatedOnRaw": entry.CreatedDateTime,
                # "MDT": entry.ModifyDateTime,
                "MD": entry.ModifyDateTime.strftime("%Y-%m-%d"),
                "MT": entry.ModifyDateTime.strftime("%I:%M %p"),
                "id": entry.id,
                "Type": "M",   
                "Url": f"https://hotelops.in:8080/PADP/api/PADP_View_Manager/?ID={entry.id}&Page=PADPApprove"   
            })
...
class Send_Notification_PADP_Approve_Mobile_API(APIView):
    """
    PADP Approve API
    Supports:
    - GET  → Fetch PADP data
...
# def get_organization_hr_emails(OID):
#     return list(
...
# def get_organization_hr_email(request, OID):
#     # OID = request.GET.get('OID')

#     if not OID:
#         return JsonResponse(
#             {"status": "error", "message": "OID is required"},
#             status=400
#         )
...
#         OfficialEmailAddress=""
#     ).values_list(
#         "OfficialEmailAddress", flat=True
#     )
