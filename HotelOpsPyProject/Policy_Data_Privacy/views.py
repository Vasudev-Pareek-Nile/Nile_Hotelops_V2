...
from .models import Data_Privacy_Employee_Detail,Data_Privacy_Deleted_File_Of_Employee, Letter_Editor
from django.shortcuts import render,redirect
...
def Data_Privacy_Entry_Emp(request):
    ...
    if EmpCode is not None:
        if LOE_ID is not None:
            Experienceobj = Data_Privacy_Employee_Detail.objects.filter(id=LOE_ID, emp_code=EmpCode, OrganizationID=OrganizationID, IsDelete=False).first()
        else:
            Experienceobj = Data_Privacy_Employee_Detail.objects.filter(emp_code=EmpCode, OrganizationID=OrganizationID, IsDelete=False).first()
    ...
    if request.method == "POST":
        ...
        if DataFromExperienceobj == "Experienceobj" and LOE_ID:
            ...
            Experienceobj = Data_Privacy_Employee_Detail.objects.create(
                emp_code=emp_code,
                prefix=prefix,
        ...
        if Page == 'Data_Privacy_Emp_List':
            return redirect('Data_Privacy_Emp_List')
    ...
    return render(request, "Policy_Data_Privacy/Entry_Emp.html", context)



from django.db.models import Subquery, OuterRef
from app.views import OrganizationList
def Data_Privacy_Emp_List(request):
    ...
    ExperienceLetters = Data_Privacy_Employee_Detail.objects.filter(IsDelete=False,OrganizationID= I).annotate(
    ...
    return render(request,"Policy_Data_Privacy/Emp_ListPage.html",context)

# For generate_appointment_letter
def Generate_Letter_of_Data_Privacy(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)

    id = request.GET.get("LOE_ID")
    OID = request.GET.get("OID")
    emp = Data_Privacy_Employee_Detail.objects.get(id=id)
    policy = Letter_Editor.objects.filter(OrganizationID=OID, IsDelete=False, Letter_Data_Item_Master_id=2).first()
    ...
    template = get_template("Policy_Data_Privacy/Letter_Of_Data_Privacy.html")
...
def Data_Privacy_Emp_Delete(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    EmpID = request.GET.get('EmpID')
    id = request.GET.get('LOE_ID') 
    empdetail = Data_Privacy_Employee_Detail.objects.get(id=id)
...
def Data_Privacy_Upload_File(request):
    if 'OrganizationID' not in request.session:
        return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    return render(request, "Policy_Data_Privacy/Upload_File.html")


def Data_Privacy_Download_File(request):
    id = request.GET.get('LOE_ID')
    OID  = request.GET.get('OID')
    if OID:
            OrganizationID= OID   
    file = Data_Privacy_Employee_Detail.objects.get(pk=id)
...
def Data_Privacy_Repalce_File(request):
    if 'OrganizationID' not in request.session:
            return redirect(MasterAttribute.Host)
    else:
        OrganizationID =request.session["OrganizationID"]
    ...
    file_id = file.file_id
    file_name = file.file_name
    EmployeeDetail = Data_Privacy_Employee_Detail.objects.get(pk=id)
    
    
    deletefile = Data_Privacy_Deleted_File_Of_Employee.objects.create(Data_Privacy_Employee_Detail= EmployeeDetail,file_id = file_id,file_name = file_name)
...
def Editor_List_View(request):
    ...
    # selected filters
    SelectedOrg = request.GET.get("OID", OrganizationID)
    SelectedPolicyID = request.GET.get("PolicyID", "")

    # base query
    policies = Letter_Editor.objects.filter(
    ...
    # apply Policy filter
    if SelectedPolicyID:
        policies = policies.filter(Letter_Data_Item_Master_id=SelectedPolicyID)

    Item_Master = Letter_Data_Item_Master.objects.filter(IsDelete=False)
    memOrg = OrganizationList(OrganizationID)
    ...
    return render(request, "Policy_Data_Privacy/Editor_List.html", context)


def Editor_Edit_View(request, pid):
    OrganizationID = request.session.get("OrganizationID")
    SessionUserID = str(request.session["UserID"])
    policy = Letter_Editor.objects.get(id=pid)
    Item_Master = Letter_Data_Item_Master.objects.filter(IsDelete=False)
    

    if request.method == "POST":
        policy.HotelID = request.POST.get("OID") or OrganizationID
        # policy.Letter_Data_Item_Master = request.POST.get("PolicyID") or ''

        # policy.name = request.POST.get("PolicyName")
        policy.Letter_Data_Item_Master_id = request.POST.get("PolicyID") or None
        policy.content = request.POST.get("PolicyContent")
        policy.ModifyBy = SessionUserID
        policy.ModifyDateTime = timezone.now()
    ...
    return render(request, "Policy_Data_Privacy/Editor_Create_Edit_Page.html", context)



def Editor_Create_View(request):
    OrganizationID = request.session.get("OrganizationID")
    SessionUserID = str(request.session["UserID"])
    
    if request.method == "POST":
        # name = request.POST.get("PolicyName") or ''
        PolicyID = request.POST.get("PolicyID") or ''
