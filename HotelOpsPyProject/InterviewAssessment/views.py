...
def view_file(request):
    ...
    if model_name == 'Assessment_Master':
        try:
            ...
            # file = Assessment_Master.objects.get(id=int(ID))
...
def LevelWiseGrid(request):
    ...
    appliedFor = request.GET.get('appliedFor')
...
def CandidateDataForm(request):
    # print("We are at(@) CandidateDataForm")
    ID = request.GET.get('ID')
    ...
    if request.method == "POST":
                ...
                url = reverse(Nexturl)  
                # print("candidate data from next link:", Nexturl)
                redirect_url = f"{url}?EMPID={EMPID}&IAID={InterviewID}" 
                return redirect(redirect_url)
    context= {
        'Emobj':Emobj,
        'EMPID':EMPID,
...
def CandidatePersonalData(request):
    ...
    # print("InterviewID is here @ CandidatePersonalData::", InterviewID)
    ...
    if request.method == "POST":
                ...
                url = reverse(Nexturl)  
                # print("candidate personal data from next link:", Nexturl)
                redirect_url = f"{url}?EMPID={EMPID}&IAID={InterviewID}" 
                return redirect(redirect_url)
...
def CandidateFamilyinfoPage(request):
    ...
    EMPID = request.GET.get('EMPID')
    InterviewID = request.GET.get('IAID')
    # print("InterviewID is here @ CandidateFamilyinfoPage::", InterviewID)
    ...
    if request.method == "POST":
                ...
                if Finfoobj  is not None:
                    ...
                    Finfo =  EmployeeFamilyData.objects.create(
                            MasterID = EMPID,
...
def CandidateEmergencyinfoPage(request):
    ...
    if request.method == "POST":
                ...
                if Emergencyobj  is not None:
                    ...
                    EmployeeEmergencyInfoData.objects.create(
                        MasterID =  EMPID,
...
def CandidateQualificationinfoPage(request):
    EMPID = request.GET.get('EMPID')
    InterviewID = request.GET.get('IAID')
    # print("InterviewID is here @ CandidateQualificationinfoPage::", InterviewID)
    Emobj = None
    Educations = None
    ActiveLinkDict = None
    ...
    if request.method == "POST":
        ...
        url = reverse(Nexturl)
        # print("qualification next url:", Nexturl)
        redirect_url = f"{url}?EMPID={EMPID}&IAID={InterviewID}"
        # print("IA id at exit and start previous work info:", InterviewID)
        return redirect(redirect_url)
...
def CandidatePreviousWorkinfoPage(request):
    EMPID = request.GET.get('EMPID')
    InterviewID = request.GET.get('IAID')
    # print("InterviewID is here @ CandidatePreviousWorkinfoPage::", InterviewID)
    Emobj = None
    previousworks = None
    ActiveLinkDict = None
...
def CandidateAddressinfoPage(request):
    EMPID = request.GET.get('EMPID')
    InterviewID = request.GET.get('IAID')
    # print("InterviewID is here @ CandidateAddressinfoPage::", InterviewID)
    Emobj = None
    Addressinfo = None 
    ActiveLinkDict = None
    ...
    if request.method == "POST":
                ...
                if Addressinfo is not None  :
                    ...
                    EmployeeAddressInfoData.objects.create(MasterID =  EMPID,Permanent_Address  = Permanent_Address,Permanent_City = Permanent_City,Permanent_State = Permanent_State,Permanent_Pincode = Permanent_Pincode,Permanent_HousePhoneNumber = Permanent_HousePhoneNumber,Temporary_Address = Temporary_Address,Temporary_City =Temporary_City,Temporary_State = Temporary_State,Temporary_Pincode=Temporary_Pincode,Temporary_HousePhoneNumber=Temporary_HousePhoneNumber)
...
def CandidateIdentityinfoPage(request):
    EMPID = request.GET.get('EMPID')
    InterviewID = request.GET.get('IAID')
    # print("InterviewID is here @ CandidateIdentityinfoPage::", InterviewID)
    Emobj = None
    IdentityInfo = None 
    ActiveLinkDict = None
    ...
    if request.method == "POST":
                ...
                if IdentityInfo is not None:
                    ...
                    IdentityInfo =  EmployeeIdentityInfoData.objects.create(   
                                        MasterID = EMPID,
                                        PANNo = PANNo,
...
def CandidateBankinfoPage(request):
    EMPID = request.GET.get('EMPID')
    InterviewID = request.GET.get('IAID')
    # print("InterviewID is here @ CandidateBankinfoPage::", InterviewID)
    Emobj = None
    Bankinfo = None 
    ActiveLinkDict = None
    ...
    if request.method == "POST":
        ...
        if Bankinfo is not None:
            ...
            Bankinfo = EmployeeBankInfoData.objects.create(
                MasterID =  EMPID,
...
def CandidateDocumentinfoPage(request):
    EMPID = request.GET.get('EMPID')
    InterviewID = request.GET.get('IAID')
    # print("InterviewID is here @ CandidateDocumentinfoPage::", InterviewID)
    Emobj = None
    Documents = None 
    ActiveLinkDict = None
    ...
    if EMPID is not None and EMPID.isdigit():
        ...
        if Emobj:
                ...
                # print("EMPID is here:", EMPID)
                if assobj.IsEmpDataReceived == True:
                    # print("IsEmpDataReceived::", assobj.IsEmpDataReceived)
                    case_type = "data_submitted"
                    return redirect('notification_page', case_type=case_type) 
