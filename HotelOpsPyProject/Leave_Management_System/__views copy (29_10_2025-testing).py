...
def Employee_Dashboard(request):
    ...
    Selected_Year = request.GET.get('year', datetime.now().year)  # Default to current year
    Selected_Month = request.GET.get('month', datetime.now().month)  # Default to current month
            ...
            with transaction.atomic():
                if request.method =="POST":
                    if leave_id is not None:
                        ...
                        # print("the latest created object of leave is here::", application)
                        # print("the latest leave id::", application.id)

                        Send_Live_Notification(
                            organization_id=OrganizationID,
                            EmpCode=EmployeeCode,
                            title=f"Hello Gourav 2525",
                            message="Sir Hello my namge is vasudev",
                            module_name="LeaveManagementSystem",
                            action="CREATE",
                            hops_id="application.id",
                            user_type="admin",
                            priority="high"
                        )
                        ...
                        #     Title = f"Cong. you send notification, id={application.id}",
                        #     Message = 'My name is vasudev',
                        #     OrgID = 3,
                        #     ModuleName='Leave Application Management',
                        #     action="CREATE",
                        #     user_type="admin",
                        #     priority="high"
                        # )
                ...
                while current_date <= LeaveEndDate:
                    ...
                    objAtt, created = Attendance_Data.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            'ActionDateTime':  datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                        }
                    )
    ...
    if request.method == 'POST':
        ...
        if ApplicationID is not None:
            with transaction.atomic():
                ...
                approve.ActionDateTime = datetime.now()
